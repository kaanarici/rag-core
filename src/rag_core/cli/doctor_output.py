...
def emit_doctor(payload: dict[str, object], *, as_json: bool, fix: bool) -> None:
    ...
    print(f"Pipeline Version: {payload.get('pipeline_version')}")
    source_versions = payload.get("source_pipeline_versions")
    if isinstance(source_versions, dict):
        print("Source Pipeline Versions:")
...
def _emit_vector_store_provider_summary(vector_store: dict[str, object]) -> None:
    ...
    for provider_name in BUILTIN_VECTOR_STORE_PROVIDER_ORDER:
        ...
        maturity = provider.get("maturity") or "unknown"
        query_plan = _query_plan_stage_summary(provider.get("query_plan"))
        print(
            f"  {marker} {provider_name}: "
            f"maturity={maturity} query_plan={query_plan}"
        )
...
def _emit_provider_health(payload: dict[str, object]) -> None:
    provider_health = require_mapping(payload.get("provider_health"))
    if not provider_health:
        return
    for key, label in (("embedding", "Embedding"), ("reranker", "Reranker")):
        health = require_mapping(provider_health.get(key))
        if not health:
            continue
        print(
            f"{label} Health: "
            f"healthy={health.get('healthy')} "
            f"provider={health.get('adapter')} "
            f"model={health.get('model') or 'none'}"
        )
        if health.get("healthy") is False:
            print(
                f"  -> error={health.get('error')} "
                f"category={health.get('error_category')} "
...
def _emit_provider_category_summary(
    ...
    for provider_name in ordered:
        ...
        print(
            ...
            f"maturity={maturity} package={package} api_key={api_key}{suffix}"
        )
...
def _doctor_next_steps(payload: dict[str, object]) -> list[str]:
    ...
    if _embedding_api_key_missing(configured_embedding, embedding_provider):
        steps.append(
            "For local semantic retrieval, run "
            '`rag-core search "How can invoices be paid?" examples/demo_corpus`.'
        )
