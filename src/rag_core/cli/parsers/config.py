...
def add_config_flags(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        help="Physical Qdrant collection. Logical retrieval scope uses --collection.",
    )
    parser.add_argument(
        "--embedding-provider",
    ...
    parser.add_argument(
        "--pipeline-version",
        dest="processing_version",
        default=env_or_default(PROCESSING_VERSION_ENV, DEFAULT_PROCESSING_VERSION),
        help="Base pipeline version used for automatic reindex decisions.",
    )
    parser.add_argument(
        "--processing-version",
        dest="processing_version",
