...
from rag_core.ingest.urls.records import safe_remote_output_source_url
...
def emit_ingested_document(document: IngestedDocument, *, as_json: bool) -> None:
    ...
    _rename_processing_version(payload)
...
def _rename_processing_version(payload: dict[str, object]) -> None:
    processing_version = payload.pop("processing_version", None)
    if processing_version is not None:
        payload["pipeline_version"] = processing_version
