...
from rag_core.cli.commands.archive import run_ingest_archive_command
...
from rag_core.cli.commands.url_ingest import run_ingest_url_command, run_ingest_urls_command
...
async def run_ingest_command(
    ...
    *,
    ...
    def local_core_factory() -> Engine:
        config = Config.from_cli(args, manifest_dir=manifest_dir)
...
def _route_ingest_source(args: argparse.Namespace) -> IngestRoute:
    ...
    if source is None:
        raise ValueError(
            "add requires a source path/URL or --url-list <file.txt>"
        )
    ...
    raise FileNotFoundError(
        "add source must be an existing file or directory, a glob, a .zip "
