...
from rag_core.local_search.eval_runner import LocalEvalCore, LocalEvalCoreFactory, run_local_eval
...
async def run_local_eval_command(
    ...
    *,
    ...
    if event_sink is not None:
        ...
        from rag_core.local_search.models import DEFAULT_LOCAL_SEARCH_COLLECTION

        def core_factory() -> LocalEvalCore:
            return build_demo_core(
                store_collection=DEFAULT_LOCAL_SEARCH_COLLECTION,
...
def _emit_local_eval(payload: EvalReport, *, as_json: bool) -> None:
    ...
    print(f"Corpus: {run.get('namespace')}/{run.get('collection')}")
