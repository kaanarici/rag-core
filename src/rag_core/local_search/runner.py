...
async def _search_local_documents(
    ...
    search_hits = getattr(core, "_search_hits", None)
    if callable(search_hits):
        result = await search_hits(
            query=run_spec.query,
            namespace=run_spec.namespace,
            collections=[run_spec.collection],
            limit=run_spec.limit,
            rerank=False,
            query_plan=search_profile(DEFAULT_SEARCH_PROFILE, limit=run_spec.limit),
        )
    else:
        legacy_search = getattr(core, "search", None)
        if not callable(legacy_search):
            raise TypeError("local search core does not expose a search method")
