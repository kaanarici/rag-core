...
def discovery_max_urls(value: int | None, *, default: int) -> int:
    resolved = default if value is None else value
    if resolved <= 0:
        raise ValueError("--max-urls must be positive")
    return resolved
...
def remote_discovery_reader(
    *,
    policy: FetchSecurityPolicy,
    limits: FetchLimits,
) -> RemoteDiscoveryReader:
    return RemoteDiscoveryReader(policy=policy, limits=limits)
