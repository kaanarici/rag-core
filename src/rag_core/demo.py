...
def build_demo_core(
    *,
    ...
    return Engine(
        ...
        sparse_embedder=DemoSparseEmbedder() if sparse else None,
...
def _tokens(text: str) -> list[str]:
    return [_normalize_demo_token(token) for token in _TOKEN_RE.findall(text.lower())]


def _normalize_demo_token(token: str) -> str:
    if token in {"paid", "payment", "payments"}:
        return "pay"
    if token in {"billing", "bills"}:
        return "invoice"
