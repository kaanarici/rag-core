...
from rag_core.manifest_entries import ManifestReadError, entry_from_json_line, entry_to_dict
...
class _ManifestIndex:
    ...
    by_document_id: dict[str, CorpusManifestEntry]


def append_manifest_jsonl_entry(path: Path, entry: CorpusManifestEntry) -> None:
...
def append_manifest_jsonl_entry_if_stale(
    ...
    is_stale: Callable[[CorpusManifestEntry | None, CorpusManifestEntry], bool],
) -> bool:
    _reject_manifest_io_paths(path)
    _ensure_private_manifest_parent(path)
    with _manifest_lock(path, exclusive=True):
...
def read_manifest_jsonl_entries(path: Path) -> list[CorpusManifestEntry]:
    _reject_manifest_io_paths(path)
...
def update_manifest_jsonl_entries(
    path: Path,
    transform: Callable[[list[CorpusManifestEntry]], Sequence[CorpusManifestEntry]],
) -> tuple[int, int, bool]:
    _reject_manifest_io_paths(path)
    _ensure_private_manifest_parent(path)
    with _manifest_lock(path, exclusive=True):
...
def _read_manifest_jsonl_entries_unlocked(path: Path) -> list[CorpusManifestEntry]:
...
def _manifest_entries_content(entries: Sequence[CorpusManifestEntry]) -> str:
    content = "\n".join(_manifest_entry_json(entry) for entry in entries)
    return f"{content}\n" if content else ""


def _manifest_entry_json(entry: CorpusManifestEntry) -> str:
...
def _cached_latest_entries_unlocked(path: Path) -> dict[str, CorpusManifestEntry]:
...
def _update_cached_index_after_append(path: Path, entry: CorpusManifestEntry) -> None:
    cached = _PROCESS_INDEXES.get(str(path))
    if cached is None:
        return
