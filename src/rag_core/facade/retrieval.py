...
from rag_core.scope import Scope, normalize_namespace, resolve_collections_argument
from rag_core.search import Context, RerankBudget, RetrievalResult, SearchResult
...
class _EngineRetrievalMethods:
    ...
    async def search(
        ...
        *,
        ...
        """Return ranked, scoped, duplicate-aware evidence."""
        from rag_core._engine.core_retrieval import retrieve_with_core

        return await retrieve_with_core(
            search=self._search,
            query=query,
            namespace=scope.namespace,
            collection=scope.collection,
            document_ids=(
                list(scope.document_ids) if scope.document_ids is not None else None
            ),
            limit=limit,
