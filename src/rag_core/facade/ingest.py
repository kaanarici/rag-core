...
from rag_core.scope import Scope, normalize_collection, normalize_namespace
...
class _EngineIngestMethods:
    ...
    async def ingest(
        ...
        *,
        ...
        raw_status = ingested.ingest_state
        if raw_status in {"replaced", "reindexed"}:
            status: IngestStatus = "replaced"
        elif raw_status == "created":
            status = "created"
        elif raw_status == "unchanged":
            status = "unchanged"
        else:
