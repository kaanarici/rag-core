...
class _RAGCoreIngestMethods:
    ...
    async def delete_corpus(
        ...
        *,
        ...
        """Explicit corpus-wide delete (every document in the corpus).

        Separate from ``delete_document`` so callers cannot reach a
        corpus-wide delete by accident (e.g. by passing an empty
        ``document_id``). The ``DeleteFilter`` seam rejects the silent
        path; this is the deliberate one.
        """
        await self._ingest.delete_corpus(
            namespace=namespace,
            corpus_id=corpus_id,
        )
    ...
    async def delete_namespace(
        ...
        *,
        ...
        and irreversible. Kept separate from the corpus path so it has its
        own contract surface and its own audit hook.
        """
