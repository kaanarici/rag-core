    ...
    def __init__(
        ...
        *,
        ...
        if embedding is not None and embedding_provider is not None:
            raise TypeError("Engine got both embedding and embedding_provider")
        if store is not None and vector_store is not None:
            raise TypeError("Engine got both store and vector_store")
        resolved_embedding = embedding or embedding_provider
        resolved_store = store or vector_store
        if config is None:
            if resolved_embedding is None or resolved_store is None:
                raise TypeError("Engine requires Config, or both embedding= and store=")
            config = Config()
        self._config = config
