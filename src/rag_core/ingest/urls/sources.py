...
from rag_core.ingest.sources.local import is_multi_link_regular_file, path_has_symlink_segment
from rag_core.ingest.urls.document_keys import private_remote_document_key
from rag_core.ingest.urls.models import RemoteUrlIngestRequest, RemoteUrlSourceItem
        ...
        return (
            tuple(
                _read_url_values(
                    ...
                    error_label="item",
                    licensed_source_registry=request.licensed_source_registry,
...
def _read_url_file(
    ...
    *,
    ...
    licensed_source_registry: LicensedSourceRegistry | None = None,
    ...
    with path.open(encoding="utf-8") as handle:
        return _read_url_values(
            handle,
            policy=policy,
            error_label="line",
            licensed_source_registry=licensed_source_registry,
    ...
    *,
    ...
    for line_number, line in enumerate(values, 1):
        ...
        try:
            ...
            raise ValueError(
                f"URL list {error_label} {line_number}: {error_message(exc)}"
            ) from exc
    return items
...
def _source_item(
    ...
    *,
    ...
    if licensed_source_registry is not None:
        if collection is None or not collection.strip():
            raise ValueError(
                "licensed_source_registry requires a non-empty collection"
            )
        licensed_source_registry.assert_host_allowed(
            collection=collection, host=validated.host
        )
    raw = url.strip()
