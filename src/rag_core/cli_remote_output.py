...
def emit_remote_discovery(payload: dict[str, object], *, as_json: bool) -> None:
    if as_json:
        print(json.dumps(payload, indent=2, sort_keys=True))
        return
    print(
        f"Discovery: {_display_discovery_kind(payload.get('source_kind'))} "
        f"({payload.get('item_count')} URLs)"
    )
    url_file = payload.get("url_file")
    ...
    for raw_item in items:
        ...
        print(f"- {item.get('url')}{label}{context}")
