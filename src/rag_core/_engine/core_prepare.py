...
async def apply_ocr(
    *,
    ...
    if is_registered_pdf_document(
        filename=parsed.filename,
        mime_type=parsed.mime_type,
    ):
        processed_entire_document = bool(
            ocr_result.metadata.get("ocr_processed_entire_document")
        )
        resolved_page_count = (
            resolve_document_page_count(
            ...
            if processed_entire_document and not processed_pages
            else None
        )
        expected_page_indices = (
            ...
            if processed_pages
                ...
                list(range(resolved_page_count))
                if resolved_page_count is not None and resolved_page_count > 0
                else page_indices
            )
        )
        expected_page_numbers = [page_index + 1 for page_index in expected_page_indices]
        if not expected_page_numbers:
            raise ValueError("OCR page boundaries could not be bound to PDF pages")
        normalized_ocr_markdown = canonicalize_pdf_page_markdown(
            ocr_result.markdown,
            expected_page_numbers,
        )
        if normalized_ocr_markdown is None:
            raise ValueError("OCR page boundaries did not match processed PDF pages")
        ocr_result = replace(ocr_result, markdown=normalized_ocr_markdown)
    ...
    document_page_count = resolve_document_page_count(
        parsed_metadata=parsed.metadata,
        ocr_metadata=ocr_result.metadata,
    )
