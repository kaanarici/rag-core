...
from rag_core.manifest.persistence import manifest_path, write_entry, write_entry_if_stale
...
class CoreIngestor:
    ...
    async def delete_collection(self, *, namespace: str, collection: str) -> None:
        await delete_collection_via_indexer(
            indexer=self._indexer, namespace=namespace, collection=collection,
        )
    ...
    async def ingest_bytes(
        ...
        *,
        ...
        if self._manifest_directory is not None:
            manifest_path(self._manifest_directory, namespace, collection)
        ...
        # (namespace, collection, document_id) so the resume-stale-delete /
        ...
        *,
        ...
        with stage_guard(sink, stage="ingest"):
            decision = await resolve_ingest_decision(
                ...
                document_id=identity.document_id,
                content_sha256=identity.content_sha256,
                ingest_fingerprint=identity.ingest_fingerprint,
