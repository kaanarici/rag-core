...
class _InFlightArchiveBytes:
    ...
    async def acquire(self, byte_count: int) -> None:
        weight = max(1, byte_count)
        async with self._condition:
            await self._condition.wait_for(lambda: self._used + weight <= self._limit)
            self._used += weight
...
async def ingest_archive_items(
    *,
    ...
    in_flight_bytes = _InFlightArchiveBytes(
        min(limits.max_entry_bytes, limits.max_total_bytes)
    )
    ...
    async def ingest_item(position: int, item: ArchiveSourceItem) -> None:
        ...
        try:
            await in_flight_bytes.acquire(item.byte_count)
            try:
                file_bytes = await asyncio.to_thread(archive.read_item, item)
                ...
                await in_flight_bytes.release(item.byte_count)
    ...
    with ZipArchiveReadSession(items[0].archive_path, limits=limits) as archive:
