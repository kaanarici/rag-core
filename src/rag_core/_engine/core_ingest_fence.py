...
triples don't accumulate, so a large corpus does not leak memory.
"""
...
# Triple identity: ``(namespace, corpus_id, document_id)``. The same tuple
...
# corpus_id casing), they're a bug in the caller, not in the fence.
...
class DocumentFence:
    ...
    Use as ``async with fence.acquire(namespace=..., corpus_id=..., document_id=...):``.
    """
    ...
    async def acquire(
        ...
        *,
        ...
        key: DocumentKey = (namespace, corpus_id, document_id)
