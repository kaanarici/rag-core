...
def resolve_collection_name(
    *,
    ...
    # Per-tier physical isolation: when CorpusPolicy binds the process to one
    # corpus tier, the slug lands between the base and the embed-model marker
    # so 'rag_core_chunks__restricted__text_embedding_3_large_3072d' cannot
    # collide with the public tier. Multi-tier or unrestricted processes pass
    # ``corpus_slug=None`` and keep the legacy single-collection name.
    if not dimension_aware:
        if corpus_slug:
            return f"{base_name}__{corpus_slug}"
        return base_name
    model_slug = re.sub(r"[^a-z0-9]+", "_", model_name.strip().lower()).strip("_")
    if corpus_slug:
        return f"{base_name}__{corpus_slug}__{model_slug}_{dimensions}d"
...
def resolve_runtime_collection_name(
    *,
    ...
    if config.vector_store.provider == QDRANT_VECTOR_STORE_PROVIDER:
        qdrant_corpus_slug: str | None = None
        if config.corpus_policy is not None:
            qdrant_corpus_slug = config.corpus_policy.collection_corpus_slug
        return resolve_collection_name(
            ...
            corpus_slug=qdrant_corpus_slug,
        )
    if config.vector_store.provider == PGVECTOR_VECTOR_STORE_PROVIDER:
        table_name = config.vector_store.pgvector.table
        if table_name:
            return table_name
        pgvector_corpus_slug: str | None = None
        if config.corpus_policy is not None:
            pgvector_corpus_slug = config.corpus_policy.collection_corpus_slug
        return resolve_collection_name(
            base_name=config.qdrant.collection,
            model_name=model_name,
            dimensions=dimensions,
            dimension_aware=config.qdrant.dimension_aware_collection,
            corpus_slug=pgvector_corpus_slug,
        )
