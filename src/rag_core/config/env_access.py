...
def get_env_int_strict(name: str, default: int) -> int:
    """Parse env var as int; raise on invalid values."""
    raw = get_env(name)
    if raw is None:
        return default
    try:
...
def get_env_float_strict(name: str, default: float) -> float:
    """Parse env var as float; raise on invalid values."""
    raw = get_env(name)
    if raw is None:
        return default
    try:
...
def get_env_bool_strict(name: str, default: bool) -> bool:
    """Parse env var as bool; raise on invalid values."""
    raw = get_env(name)
    if raw is None:
        return default
