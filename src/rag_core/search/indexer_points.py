"""Build vector-store points (and section locators) for an indexed document."""
...
from rag_core.search.vector_models import ContentType, SparseVector, VectorPoint
from .indexer_prepare import PreparedIndexData
    ...
    for index, chunk in enumerate(prepared.chunks):
        ...
        primary_sparse = (
            primary_sparse_channel(
                sparse_channels,
                missing_message=(
                    "Sparse embedding is missing for chunk index %d" % index
                ),
            )
            if sparse_channels
            else SparseVector(indices=[], values=[])
        )
...
def _build_payload(
    *,
    ...
    return build_stored_payload(
        ...
        ingest_fingerprint=req.ingest_fingerprint,
