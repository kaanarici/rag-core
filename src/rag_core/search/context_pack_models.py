"""Context dataclasses and text projection for retrieval results.
...
ContextOrder = Literal["rank", "extrema"]
CONTEXT_ORDER_RANK: Final[ContextOrder] = "rank"
CONTEXT_ORDER_EXTREMA: Final[ContextOrder] = "extrema"
CONTEXT_ORDER_VALUES: Final[tuple[ContextOrder, ...]] = (
    CONTEXT_ORDER_RANK,
    CONTEXT_ORDER_EXTREMA,
)


def validate_context_order(value: object, *, field: str = "context_order") -> ContextOrder:
    if value == CONTEXT_ORDER_RANK:
        return CONTEXT_ORDER_RANK
    if value == CONTEXT_ORDER_EXTREMA:
        return CONTEXT_ORDER_EXTREMA
...
class SourceLocator:
    ...
    ``start_offset`` / ``end_offset`` are the canonical char-offsets in the
    converted markdown, preserved end-to-end so the caller's EvidenceSpan
    can resolve back to coordinates on the rag-core-owned bytes.
    """
...
class Context:
    ...
    def as_prompt_text(
        self,
        *,
        context_order: ContextOrder = CONTEXT_ORDER_RANK,
    ) -> str:
        """Return prompt-safe text with rank-local citation ids for model input."""
        rendered = "\n\n".join(
            snippet.as_prompt_text()
            for snippet in _prompt_snippets_for_order(
                self.snippets,
...
def _source_title(source: Citation) -> str:
    return source.title or source.document_key or source.document_id or source.result_id


def _prompt_safe_source_title(source: Citation) -> str:
...
def _prompt_snippets_for_order(
    snippets: tuple[ContextSnippet, ...],
    *,
    context_order: ContextOrder,
) -> tuple[ContextSnippet, ...]:
    order = validate_context_order(context_order)
    if order == CONTEXT_ORDER_RANK:
        return snippets
    return (*snippets[::2], *reversed(snippets[1::2]))
