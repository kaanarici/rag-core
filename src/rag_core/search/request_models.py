...
class DeleteFilter:
    ...
    ``corpus_id`` previously silently widened the delete (e.g. a caller
    intended to remove a single document, mis-formatted the id, and ended
    up clearing the corpus or namespace). Empty strings are now rejected;
    callers wanting a corpus-wide or namespace-wide delete must pass
    ``None`` deliberately (and the engine routes those through the
    explicit ``delete_corpus`` / ``delete_namespace`` helpers in
    ``facade/retrieval.py``).
    """
    ...
    def __post_init__(self) -> None:
        ...
        if self.corpus_id is not None and (
            not isinstance(self.corpus_id, str) or not self.corpus_id.strip()
        ):
            raise ValueError(
                "DeleteFilter.corpus_id must be omitted (None) or a non-empty string; "
        ...
        if self.corpus_id is None and self.document_id is None:
            raise ValueError(
                "DeleteFilter requires at least one of corpus_id or document_id; "
