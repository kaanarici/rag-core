...
DuplicatePolicy = Literal["collapse", "preserve"]


@dataclass(frozen=True)
class SearchOptions:
    """Ordinary retrieval choices; query plans remain internal."""
    ...
    answerability_calibration: str | None = None

    def __post_init__(self) -> None:
        if self.mode not in {"dense", "lexical", "hybrid"}:
            raise ValueError("SearchOptions.mode must be 'dense', 'lexical', or 'hybrid'")
        if not isinstance(self.rerank, bool):
            raise ValueError("SearchOptions.rerank must be a boolean")
        if self.duplicate_policy not in {"collapse", "preserve"}:
            raise ValueError(
                "SearchOptions.duplicate_policy must be 'collapse' or 'preserve'"
            )
        if self.max_results_per_document is not None:
            _require_positive_int(
                self.max_results_per_document,
                "SearchOptions.max_results_per_document",
            )
        if self.content_types is not None:
            _require_non_blank_string_items(
                list(self.content_types),
                "SearchOptions.content_types",
            )
            object.__setattr__(
                self,
                "content_types",
                tuple(value.strip() for value in self.content_types),
            )
        if (self.answerability_threshold is None) != (
            self.answerability_calibration is None
        ):
            raise ValueError(
                "SearchOptions answerability_threshold and "
                "answerability_calibration must be set together"
            )
        if self.answerability_threshold is not None and (
            isinstance(self.answerability_threshold, bool)
            or not isinstance(self.answerability_threshold, int | float)
            or not math.isfinite(float(self.answerability_threshold))
        ):
            raise ValueError(
                "SearchOptions.answerability_threshold must be a finite number"
            )
        if self.answerability_calibration is not None and (
            not isinstance(self.answerability_calibration, str)
            or not self.answerability_calibration.strip()
        ):
            raise ValueError(
                "SearchOptions.answerability_calibration must be a non-empty string"
            )
