...
from rag_core.search.document_records import INGEST_FINGERPRINT_PAYLOAD_KEY
...
JsonPayloadKey: TypeAlias = str | int | float | Enum
...
SECTION_PAYLOAD_KEYS = (
    ...
    "is_full_page",
    "anchor_chunk_index",
    "offset_reconstruction",
)

# Chunker-owned char span keys; always emitted from PreparedChunk regardless of
# section_info presence so EvidenceSpans round-trip through upsert -> search.
SPAN_PAYLOAD_KEYS = ("start_char", "end_char")
...
def validate_json_payload(
    payload: Mapping[str, object] | Mapping[JsonPayloadKey, object],
) -> dict[str, object]:
    return {_payload_key(key): json_payload_value(value) for key, value in payload.items()}


def json_payload_value(value: object) -> object:
    if isinstance(value, Enum):
        return value.value
    if value is None or isinstance(value, (str, int, float, bool)):
        return value
    if isinstance(value, Mapping):
        return {_payload_key(key): json_payload_value(nested) for key, nested in value.items()}
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes, bytearray)):
        return [json_payload_value(nested) for nested in value]
    raise ValueError(
...
def build_stored_payload(
    *,
    ...
    # Stamp char-offsets straight off the PreparedChunk so EvidenceSpans
    # survive into the stored payload even when no section_info is attached.
    if isinstance(chunk_start_char, int) and not isinstance(chunk_start_char, bool):
        payload["start_char"] = chunk_start_char
    if isinstance(chunk_end_char, int) and not isinstance(chunk_end_char, bool):
        payload["end_char"] = chunk_end_char
    if not section_info:
        return payload
    ...
    for key in SECTION_PAYLOAD_KEYS:
        ...
        if value is not None:
            payload[key] = value
    # section_info may carry per-format char spans (e.g. row-bound XLSX
    # snippets); those override only when the chunker did not supply them.
    for key in SPAN_PAYLOAD_KEYS:
        value = section_info.get(key)
...
def payload_to_result(
    *,
    ...
    return SearchResult(
        ...
        collection=optional_payload_str(payload, policy.collection_field),
        ...
        start_char=optional_payload_int(payload, "start_char"),
        end_char=optional_payload_int(payload, "end_char"),
...
def merge_duplicate_result(
    ...
    return SearchResult(
        ...
        collection=preferred.collection or fallback.collection,
        ...
        start_char=preferred.start_char if preferred.start_char is not None else fallback.start_char,
        end_char=preferred.end_char if preferred.end_char is not None else fallback.end_char,
