    ...
    def to_store_capabilities(
        ...
        *,
        ...
        return StoreCapabilities(
            ...
            chunk_index_lookup=self.chunk_index_lookup,
...
class VectorStoreProviderSpec:
    ...
    docs_maturity: str
    docs_entrypoint: str
    diagnostic_maturity: ProviderDiagnosticMaturity
...
PGVECTOR_VECTOR_STORE_CAPABILITY_SPEC = VectorStoreCapabilitySpec(
    query_plan=QueryPlanCapabilities(
        dense=True,
    ),
    metadata_filter=MetadataFilterCapabilities(
...
QDRANT_VECTOR_STORE_PROVIDER_SPEC = VectorStoreProviderSpec(
    ...
    docs_entrypoint=(
        "`QdrantConfig`, `--qdrant-*`, `rag_core.search.providers.QdrantVectorStore`"
    ),
    diagnostic_maturity=MATURITY_DEFAULT,
...
TURBOPUFFER_VECTOR_STORE_PROVIDER_SPEC = VectorStoreProviderSpec(
    ...
    docs_entrypoint=(
        "`--store turbopuffer`, `uv sync --extra turbopuffer`, "
        "`rag_core.search.providers.turbopuffer_store.TurboPufferVectorStore`"
    ),
    diagnostic_maturity=MATURITY_OPTIONAL,
    capabilities=TURBOPUFFER_VECTOR_STORE_CAPABILITY_SPEC,
)
PGVECTOR_VECTOR_STORE_PROVIDER_SPEC = VectorStoreProviderSpec(
    name=PGVECTOR_VECTOR_STORE_PROVIDER,
    docs_label="pgvector",
    docs_maturity="first-party optional",
    docs_entrypoint=(
        "`--store pgvector`, `uv sync --extra pgvector`, "
        "`rag_core.search.providers.pgvector_store.PgVectorVectorStore`"
    ),
    diagnostic_maturity=MATURITY_OPTIONAL,
    capabilities=PGVECTOR_VECTOR_STORE_CAPABILITY_SPEC,
)
MEMORY_VECTOR_STORE_PROVIDER_SPEC = VectorStoreProviderSpec(
    ...
    docs_entrypoint=(
        "`Engine(vector_store=...)` with "
        "`rag_core.search.providers.memory_store.InMemoryVectorStore`"
    ),
    diagnostic_maturity=MATURITY_UTILITY,
