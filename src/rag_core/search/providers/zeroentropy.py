...
def _import_zeroentropy() -> "types.ModuleType":
    try:
        ...
        raise ImportError(
            ...
            "Install the zeroentropy optional extra from the same source as rag-core."
        ) from exc
    if not callable(getattr(module, "ZeroEntropy", None)):
        raise ImportError(
            "zeroentropy package is installed but incompatible: missing callable "
            "zeroentropy.ZeroEntropy constructor. Reinstall the zeroentropy optional "
            "extra from the same source as rag-core."
        )
    return module
