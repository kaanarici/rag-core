...
class InMemoryVectorStore:
    ...
    async def get_chunks_by_index(
        ...
        *,
        ...
        for stored in self._points.values():
            payload = stored.payload
            if payload.get(self._policy.namespace_field) != namespace_scoped:
                continue
            if payload.get(self._policy.collection_field) != collection_scoped:
                continue
            if payload.get(self._policy.document_id_field) != document_scoped:
                continue
            chunk_index = payload.get(self._policy.chunk_index_field)
