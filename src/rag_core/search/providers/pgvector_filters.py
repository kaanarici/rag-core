...
from rag_core.search.filters import Filter, Geo, In, Range, Term
...
def build_delete_where(
    *,
    ...
    builder = _SqlBuilder(start_index=start_index)
    filters = [_equals(policy.namespace_field, namespace, policy=policy, builder=builder)]
    if filter_values.collection is not None:
        filters.append(
            _equals(
                policy.collection_field,
...
def build_document_lookup_where(
    *,
    ...
    filters = [
        ...
        _equals(policy.collection_field, collection, policy=policy, builder=builder),
    ]
...
def _metadata_filter(
    ...
    *,
    ...
    return _PgFilterTranslator(policy=policy, builder=builder).translate(filter_node)


class _PgFilterTranslator(FilterTranslator[str]):
    def __init__(self, *, policy: VectorStorePolicy, builder: _SqlBuilder) -> None:
        self._policy = policy
        self._builder = builder

    def term(self, node: Term) -> str:
        return _equals(node.field, node.value, policy=self._policy, builder=self._builder)

    def in_(self, node: In) -> str:
        return _in_values(node.field, node.values, policy=self._policy, builder=self._builder)

    def range_(self, node: Range) -> str:
        return _range(node, policy=self._policy, builder=self._builder)

    def geo(self, node: Geo) -> str:
        return _geo(node, builder=self._builder)

    def and_(self, children: list[str]) -> str:
        return _combine_sql("AND", children)

    def or_(self, children: list[str]) -> str:
        return _combine_sql("OR", children)

    def not_(self, child: str) -> str:
        return f"(NOT {child})"

    def unsupported(self, node: Filter) -> NoReturn:
        raise TypeError(f"unsupported pgvector filter node: {type(node).__name__}")
