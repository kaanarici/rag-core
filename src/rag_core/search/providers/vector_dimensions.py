...
def validate_point_dense_dimensions(
    ...
    *,
    ...
    if dense_dimensions <= 0:
        raise ValueError("%s dense_dimensions must be positive" % backend)
    for index, point in enumerate(points):
        ...
        if actual_dimensions != dense_dimensions:
            raise ValueError(
                ...
                % (backend, index, dense_dimensions, actual_dimensions)
            )
    ...
    *,
    ...
    if actual_dimensions != dense_dimensions:
        raise ValueError(
            ...
            % (backend, dense_dimensions, actual_dimensions)
        )
