...
class NoOpReranker:
    ...
    async def check_health(self) -> ProviderHealth:
        return build_healthy_provider_health(
            ...
            model_name=DEFAULT_RERANKER_PROVIDER,
        )
...
def create_reranker(
    ...
    ``strict`` (or the ``RERANKER_STRICT_PROVIDER`` env override) raises when an
    explicitly requested provider is unavailable instead of degrading to the
    no-op reranker.
    """

    requested = (provider or DEFAULT_RERANKER_PROVIDER).strip().lower()
    strict = strict or get_env_bool("RERANKER_STRICT_PROVIDER", False)
    ...
    try:
        resolved_api_key = resolve_reranker_api_key(effective, api_key=api_key)
        reranker = RERANKER_PROVIDERS.create(
            effective,
            model=model,
...
RERANKER_PROVIDERS.register(COHERE_PROVIDER, _build_cohere_reranker)
