...
def _import_voyageai() -> "types.ModuleType":
    try:
        ...
        raise ImportError(
            ...
            "Install the voyage optional extra from the same source as rag-core."
        ) from exc
    if not callable(getattr(voyageai, "Client", None)):
        raise ImportError(
            "voyageai package is installed but incompatible: missing callable "
            "voyageai.Client constructor. Reinstall the voyage optional extra "
            "from the same source as rag-core."
        )
    return voyageai
