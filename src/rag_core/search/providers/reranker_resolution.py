...
def resolve_reranker_provider(
    ...
    if requested == COHERE_PROVIDER:
        key = _resolve_api_key(
            api_key,
            env_names=reranker_api_key_env_names(COHERE_PROVIDER),
        )
        if key:
            return COHERE_PROVIDER, None
...
def resolve_reranker_api_key(
    ...
    return _resolve_api_key(
        api_key,
        env_names=reranker_api_key_env_names(provider),
    )
