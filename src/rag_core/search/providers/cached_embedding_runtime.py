    ...
    *,
    ...
    try:
        return safe_ordered_embedding_vectors(
            ...
            provider_name=f"{_provider_name(inner)} cache",
        )[0]
    except ValueError:
        return None


def _provider_name(inner: EmbeddingProvider) -> str:
    return str(getattr(inner, "provider_name", None) or type(inner).__name__)
...
async def _embed_text_misses(
    ...
    return safe_ordered_embedding_vectors(
        ...
        expected_dimensions=inner.dimensions,
        provider_name=_provider_name(inner),
    )
