...
DEFAULT_SEARCH_PROFILE = SEARCH_PROFILE_FAST
DEFAULT_SEARCH_PROFILE_SCOPE = "all_dense_capable_stores"
DEFAULT_QUERY_PLAN_BEHAVIOR = "dense_first"
DEFAULT_SEARCH_PROFILE_NOTE = (
    f"{SEARCH_PROFILE_FAST} is the default. Lexical and hybrid retrieval are "
    "explicit opt-ins and require a sparse-capable index."
)
...
def rerank_candidate_pool_limit(
    *, final_limit: int, requested: int | None = None
) -> int:
    """Candidate count to fetch and feed a provider reranker.

    A provider reranker can only reorder what retrieval returned, so fetching
    just ``final_limit`` lets it reorder the top-K but never promote a relevant
    document ranked deeper. Widen to the same multiplier-bounded pool used for
    prefetch candidate generation, honoring an explicit requested count (a
    ``RerankBudget.candidate_count``) but never dropping below ``final_limit``.
    The final result count is unchanged: the rerank stage and the pipeline both
    slice back to ``final_limit``.
    """
    if final_limit <= 0:
        raise ValueError("limit must be positive")
    if requested is not None:
        return max(requested, final_limit)
    # Floor at final_limit: the multiplier-bounded pool caps at
    # _MAX_PREFETCH_LIMIT, which is below a large final_limit. Returning less
    # than final_limit would make a large-limit + rerank query deliver fewer
    # results than requested (the final slice is results[:query.limit]).
    return max(resolve_prefetch_limit(result_limit=final_limit), final_limit)
...
def default_query_plan_for_capabilities(
    *,
    ...
    """Choose the dense baseline unless the store is sparse-only."""
