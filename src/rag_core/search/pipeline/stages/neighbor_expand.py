...
async def _fetch_chunks_by_doc(
    ...
    for doc_key, doc_indices in indices_by_doc.items():
        namespace, collection, document_id = doc_key
...
def _requested_chunk_index(
    ...
    if (
        ...
        or chunk.collection != collection
...
def _doc_key(result: SearchResult, query: PipelineQuery) -> _DocKey | None:
    ...
    if collection is None and len(query.collections) == 1:
        collection = query.collections[0]
    document_id = result.document_id
    if not namespace or not collection or not collection.strip():
        return None
    if not document_id or not document_id.strip():
        return None
    return namespace, collection.strip(), document_id.strip()
