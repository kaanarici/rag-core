...
async def resolve_sidecar_results(
    ...
    if prefetched is not None:
        return await _await_sidecar(prefetched.task, prefetched.started_ms)
    if sidecar is None:
        return SidecarResolution(results=[], duration_ms=0.0)
    return await _await_sidecar(sidecar.search(build_sidecar_query(query)), now_ms())


async def _await_sidecar(
    awaitable: Awaitable[list[SearchResult]],
    started_ms: float,
) -> SidecarResolution:
    try:
        ...
        return SidecarResolution(results=results, duration_ms=now_ms() - started_ms)
    _log_sidecar_failure(error_type)
    return SidecarResolution(
        results=[], duration_ms=now_ms() - started_ms, error_type=error_type
    )
