...
def _document_dedupe_key(result: SearchResult) -> tuple[str, str, str, str] | None:
    ...
    if result.document_id:
        return ("document_id", namespace, result.corpus_id or "", result.document_id)
    if result.document_key and result.corpus_id:
        return ("document_key", namespace, result.corpus_id, result.document_key)
    return None
