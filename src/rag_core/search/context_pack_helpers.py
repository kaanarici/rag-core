...
def source_dedupe_key(result: SearchResult) -> SourceDedupeKey:
    ...
    if result.content_sha256 and result.chunk_index is not None:
        return (
            "content_sha256",
            result.namespace or "",
            result.collection or "",
            result.content_sha256,
            "chunk",
            str(result.chunk_index),
            "prepared_text",
            _short_hash(result.text),
            "span",
            str(result.start_char),
            str(result.end_char),
            "section",
            result.section_id or result.section_path or "",
            "chunker",
            result.chunker_strategy or "",
            *qualifier,
        )
