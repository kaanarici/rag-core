...
class _RAGCoreConfigMeta(type):
    def qdrant(
        ...
        *,
        ...
        return _rag_core_config_qdrant(
            cast("type[RAGCoreConfig]", cls),
            url=url,
...
class RAGCoreConfig(metaclass=_RAGCoreConfigMeta):
    ...
    # Optional per-process corpus/tier fence. ``None`` = unrestricted behavior.
    # Tiered deployments can pass a CorpusPolicy so rerank, lexical sidecar,
    # and cross-namespace requests fail at the seam before provider calls.
    corpus_policy: CorpusPolicy | None = None

    @classmethod
    def local(cls, persist_dir: str | Path | None = None) -> "RAGCoreConfig":
        return cls(
            qdrant=QdrantConfig(
...
def _rag_core_config_qdrant(
    cls: type[RAGCoreConfig],
    *,
    url: str,
    embedding_provider: str = DEFAULT_EMBEDDING_PROVIDER,
    model: str | None = None,
...
def _default_embedding_model(provider: str) -> str:
    ...
    raise ValueError(
        f"RAGCoreConfig.qdrant(embedding_provider={provider!r}) needs an explicit "
...
class DeleteDocumentResult:
    ...
    surface configured on this RAGCore" (``None``).
    """
    ...
    # this RAGCore (e.g. NoCache); ``True`` = scoped purge succeeded; ``False``
