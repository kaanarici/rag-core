...
class IngestJobStore:
    ...
    def _connect(self) -> sqlite3.Connection:
        ...
        # left at the default because every call opens its own connection.
        ...
        with self._connect() as connection:
            connection.execute(
                ...
                INSERT INTO ingest_jobs (
                    job_id, status, path, namespace, corpus_id, updated_at
                )
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (job_id, INGEST_JOB_STATUS_PENDING, path, namespace, corpus_id, now),
            )
