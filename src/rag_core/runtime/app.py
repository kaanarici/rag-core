...
def create_app(
    *,
    ...
    async def ingest(request: Request) -> JSONResponse:
        ...
        # events are emitted by ``RAGCore.ingest_file`` once the worker
...
def _bound_namespace_from_config(config: Any) -> str | None:
    """Extract the process-bound tenant namespace from CorpusPolicy, if set.

    Returns ``None`` for unbound deployments. The existing library-only /
    dev-server case. A bound CorpusPolicy makes the HTTP boundary refuse
    cross-namespace requests before they reach the engine.
    """
    policy = getattr(config, "corpus_policy", None)
    if policy is None:
        return None
    ...
    try:
        ...
        try:
            ...
            _logger.info(
                ...
                "job_id=%s namespace=%s corpus_id=%s document_id=%s",
                INGEST_COMPLETED_EVENT,
                audit_context.request_id if audit_context else None,
                audit_context.actor if audit_context else None,
                audit_context.ingest_id if audit_context else None,
