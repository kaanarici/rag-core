"""Payload and bucket helpers for ingest results."""
...
from rag_core.ingest_states import INGEST_STATE_UNCHANGED
...
def written_records(records: Sequence[TSuccess]) -> tuple[TSuccess, ...]:
    return tuple(
        record for record in records if record.ingest_state != INGEST_STATE_UNCHANGED
    )
