...
def _allows_empty_ocr_only_output(
    *,
    ...
    if not bool(metadata.get("needs_ocr")):
        return False
    if is_registered_pdf_document(filename=filename, mime_type=mime_type):
...
async def parse_file_bytes(
    *,
    ...
    try:
        ...
        normalized_ocr_page_indices = _normalize_ocr_page_indices(
            metadata.get("ocr_page_indices")
        )
        ...
        if result.quality:
            metadata["quality"] = quality_score_to_metadata(result.quality)
