...
from rag_core.documents.converters.registry_maps import (
    is_registered_image_document,
    is_registered_pdf_document,
)
...
from rag_core.documents.page_indices import (
    normalize_page_indices as _normalize_page_indices,
)
...
def main() -> int:
    ...
    if (
        is_registered_pdf_document(filename=filename, mime_type=mime_type)
    ...
    is_pdf = is_registered_pdf_document(filename=filename, mime_type=mime_type)
    prompt_page_indices = (
        pages_processed
        if is_pdf and whole_document_page_count > 0
        else requested_page_indices
    )
    ...
    url = f"https://{_GEMINI_API_HOST}/v1beta/models/{args.model}:generateContent"
...
def _build_prompt(page_indices: list[int], *, pdf_subset: bool = False) -> str:
    ...
    if page_indices:
        ...
        return (
            ...
            "Return only markdown. Transcribe the document and prefix each page with a "
            f"`## Page N` heading using these page numbers in order: {page_numbers}."
        )
    return (
        "Convert this document to markdown. Preserve headings, lists, tables, and links. "
...
def _load_json(req: request.Request) -> dict[str, object]:
    ...
    try:
        ...
        raise RuntimeError(f"Gemini OCR request timed out after {timeout:g}s") from exc
...
def _assert_pinned_host(url: str) -> None:
    ...
    if host != _GEMINI_API_HOST:
        raise RuntimeError(f"Gemini OCR refused unpinned host: {host or '<missing>'}")
...
def _ensure_page_heading(
    ...
    """Rebuild the owned boundary for a single known Gemini page.

    Gemini returns the whole document as one markdown blob, so multi-page
    documents cannot be partitioned reliably here. A matching leading wrapper
    is consumed once; page-like source headings remain visible but nonstructural.
    """
    if not markdown.strip():
        return markdown
    if page_count != 1:
        return markdown
    return render_owned_pdf_page(
        markdown,
        page_number=page_number,
        consume_leading_boundary=True,
    )
...
def _with_original_page_headings(markdown: str, page_indices: list[int]) -> str:
    ...
    canonical = canonicalize_pdf_page_markdown(
        markdown,
        page_numbers,
    )
    if canonical is None:
        raise ValueError("Gemini OCR page boundaries did not match processed pages")
    return canonical
...
def _subset_pdf_pages(
    ...
    with fitz.open(stream=file_bytes, filetype="pdf") as source:
        ...
        selected = _normalize_page_indices(page_indices, page_count=page_count)
