...
from rag_core.config.contextualizer_config import validate_contextualizer_chunk_cap
...
def contextualizer_chunk_cap(contextualizer: object | None) -> int | None:
    if contextualizer is None:
        return None
    return validate_contextualizer_chunk_cap(
        getattr(contextualizer, "contextualizer_chunk_cap", None)
    )
