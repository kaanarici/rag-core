...
def require_pdf_inspector_page_count(payload: dict[str, object]) -> int:
    return validate_pdf_page_count(payload.get("page_count"))
...
def parse_analysis_fields(
    ...
    if (
        is_complex is None
        and pages_with_tables is not None
        and pages_with_columns is not None
    ):
...
class PdfInspectorDetectionResult:
    ...
    has_explicit_ocr_page_info: bool = True
...
def detection_result_from_payload(
    ...
    return PdfInspectorDetectionResult(
        ...
        page_count=require_pdf_inspector_page_count(payload),
        ...
        has_explicit_ocr_page_info="pages_needing_ocr" in payload,
