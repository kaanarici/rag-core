...
def process_pdf_with_inspector_wheel(
    file_bytes: bytes,
) -> PdfInspectorProcessResult | None:
...
def extract_pdf_with_inspector(
    file_bytes: bytes,
) -> PdfInspectorExtractionResult | None:
...
def _process_wheel_result(result: object) -> PdfInspectorProcessResult:
    page_count = validate_pdf_page_count(_get_wheel_field(result, "page_count"))
    pages_needing_ocr = _wheel_ocr_page_numbers(
        result,
    ...
    if pages_needing_ocr is not None:
        payload["pages_needing_ocr"] = _page_numbers(pages_needing_ocr)
...
def _has_wheel_field(result: object, name: str) -> bool:
    if isinstance(result, Mapping):
