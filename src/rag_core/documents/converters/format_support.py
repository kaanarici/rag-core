...
def format_support_payloads() -> list[dict[str, object]]:
    return [
        {
            "key": entry.key,
            "label": entry.label,
            "maturity": entry.maturity,
            "converter_key": entry.converter_key,
            "extensions": list(entry.extensions),
            "mime_types": list(entry.mime_types),
            "local_ingest": entry.local_ingest,
            "ocr": entry.ocr,
            "notes": entry.notes,
        }
...
def unsupported_local_file_message(path: Path, *, label: str = "path") -> str:
    ...
    return (
        ...
        "See https://kaanarici.github.io/rag-core/docs/formats."
    )
