...
from rag_core.documents.xlsx_protocol import render_xlsx_sheet_marker
...
def build_xlsx_sheet_sections(
    *,
        ...
        if truncated:
            sections.append(
                "\n\n".join(
                    [
                        "## Sheet: %s" % sheet_title,
                        render_xlsx_sheet_marker(sheet_title),
    ...
    for group_index, (start_row, end_row, group_rows) in enumerate(row_groups):
        ...
        section_parts.append(_render_row_group(rows_data, group_rows))
...
def _render_row_group(
    ...
    table = render_markdown_table(
        [[_normalize_table_cell(cell) for cell in row.cells] for row in rendered_rows]
    )
    lines = table.splitlines()
    for row_index, row in enumerate(rendered_rows):
        line_index = 0 if row_index == 0 else row_index + 1
        lines[line_index] = (
            f"{lines[line_index]} <!-- rag-core-xlsx-row:{row.row_number} -->"
        )
    return "\n".join(lines)
