...
class PdfConverter(HybridConverter):
    ...
    async def _try_extract_with_inspector_wheel(
        ...
        result = self._inspector_conversion_from_results(
            ...
            page_count=_get_inspector_page_count(
                processed.extraction,
                processed.detection,
            )
            or 1,
            has_explicit_ocr_page_info=processed.has_explicit_ocr_page_info,
            adapter="wheel",
        )
    ...
    async def _try_extract_with_inspector(
        ...
        return self._inspector_conversion_from_results(
            ...
            page_count=page_count,
            has_explicit_ocr_page_info=True,
            adapter=None,
        )
    ...
    def _inspector_conversion_from_results(
        ...
        *,
        ...
        if not _inspector_is_text_based(
            ...
            self._log_inspector_fallback(route=route, page_count=page_count, adapter=adapter)
            return None
        ...
        if not _inspector_supports_page_level_routing(detection):
            self._log_inspector_fallback(route=route, page_count=page_count, adapter=adapter)
            return None
        if not has_explicit_ocr_page_info:
            logger.info(
                "PDF Inspector returned no page OCR routing; inspector_path=%s adapter=%s route=%s page_count=%d fallback_path=%s",
                _INSPECTOR_PATH,
                adapter or "cli",
                route or "unknown",
                page_count,
