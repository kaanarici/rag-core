...
class EvalResult:
    ...
    off_topic_result_rate: float = 0.0
...
async def _run_eval_case(
    ...
    *,
    ...
    try:
        hits, retrieval_diagnostics = await _eval_search(
            core,
            case,
            limit=limit,
            rerank=rerank,
    ...
    suppressed_duplicates = _diagnostic_int(
        retrieval_diagnostics,
        "duplicate_count",
    )
    candidate_count = _diagnostic_int(
        retrieval_diagnostics,
        "candidate_count",
    )
    duplicate_rate = (
        suppressed_duplicates / candidate_count
        if candidate_count > 0
        else _visible_duplicate_rate(hits)
    )
    irrelevant_count = sum(
        1 for retrieved_id in retrieved_ids if retrieved_id not in relevant_ids
    )
    return EvalResult(
        case=case,
        ...
        unique_document_count=len({_document_identity(hit) for hit in hits}),
        off_topic_result_rate=(
            irrelevant_count / result_count if result_count else 0.0
        ),
        suppressed_duplicate_count=suppressed_duplicates,
        answerability_status=str(
            retrieval_diagnostics.get("answerability_status", "unknown")
        ),
        retrieval_mode=str(retrieval_diagnostics.get("mode", "advanced")),
...
async def _eval_search(
    ...
    *,
    ...
    if (
        len(case.collections) == 1
        and query_plan is None
        and rerank_budget is None
        and callable(getattr(core, "_search_hits", None))
        and callable(getattr(core, "search", None))
    ):
        from rag_core import Scope, SearchOptions

        retrieval = await core.search(
            case.query,
            scope=Scope(
                tenant_id=case.namespace,
                collection=case.collections[0],
            ),
            limit=limit,
            options=SearchOptions(rerank=rerank),
        )
        return (
            list(retrieval.evidence),
            {
    ...
    search_hits: Any = getattr(core, "_search_hits", None)
    if not callable(search_hits):
        search_hits = getattr(core, "search")
    hits = cast(
        "list[SearchResult]",
        await search_hits(
            query=case.query,
...
def _visible_duplicate_rate(hits: Sequence[SearchResult]) -> float:
    if not hits:
        return 0.0
    identities = {
        re.sub(r"\s+", " ", hit.text).strip().casefold()
        for hit in hits
    }
    return (len(hits) - len(identities)) / len(hits)


def _document_identity(hit: SearchResult) -> str:
    return hit.document_id or hit.document_key or hit.id
