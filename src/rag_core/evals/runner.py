...
The runner consumes :class:`rag_core.core.Engine` through its owner module.
An eval case feeds the engine a query and the engine yields
...
class EvalResult:
    ...
    forbidden_id_count: int = 0
    no_result_pass: bool = True
...
def _result_id(
    ...
    *,
    ...
    known_ids = expected_ids | grade_ids | forbidden_ids
    if result.id in known_ids:
        return result.id
    if result.document_id and (result.document_id in known_ids):
        return result.document_id
    if result.document_key and (result.document_key in known_ids):
...
def _context_metrics(case: EvalCase, hits: Sequence[SearchResult]) -> dict[str, Any]:
    ...
    prompt_safety_pass = (
        ...
        and (case.max_context_chars is None or len(text) <= case.max_context_chars)
        and (
...
def _has_context_expectations(case: EvalCase) -> bool:
    return bool(
        ...
        or case.forbidden_ids
        or case.expect_no_results
    )
    ...
    *,
    ...
    try:
        ...
        return EvalResult(
            ...
            no_result_pass=not case.expect_no_results,
        )
    latency_ms = (perf_counter() - started) * 1000.0
    ...
    forbidden_id_count = sum(
        1 for result_id in retrieved_ids if result_id in forbidden_ids
    )
    no_result_pass = not hits if case.expect_no_results else True
    ...
    if case.expect_no_results:
        ...
        reciprocal_rank = mrr(retrieved_ids, relevant)
        ndcg_10 = ndcg_at_k(retrieved_ids, grades, 10)
    context_metrics = _context_metrics(case, hits)
    if forbidden_id_count:
        context_metrics["prompt_safety_pass"] = False
    return EvalResult(
        case=case,
        ...
        ndcg_at_10=ndcg_10,
        latency_ms=latency_ms,
        forbidden_id_count=forbidden_id_count,
