...
def eval_result_payload(result: EvalResult) -> EvalReport:
    ...
    payload: EvalReport = {
        ...
        "forbidden_id_count": result.forbidden_id_count,
    }
    ...
    if case.document_ids:
        payload["document_ids"] = list(case.document_ids)
    if case.forbidden_ids:
        payload["forbidden_ids"] = list(case.forbidden_ids)
    if case.expect_no_results:
        payload["expect_no_results"] = True
        payload["no_result_pass"] = result.no_result_pass
...
def eval_report(
    ...
    *,
    ...
    report: EvalReport = {
        ...
        "failure_count": sum(
            1 for payload in result_payloads if payload.get("ok") is False
        ),
...
def _redacted_case_payload(payload: object) -> EvalReport:
    ...
    redacted.pop("document_ids", None)
    redacted.pop("forbidden_ids", None)
...
def _aggregate_metrics(
    ...
    *,
    ...
    answerable = [
        ...
        if isinstance(payload.get("expected_ids"), list)
        and bool(payload["expected_ids"])
    ]
    metrics: EvalReport = {
        "answerable_case_count": len(answerable),
        "no_result_case_count": sum(
            payload.get("expect_no_results") is True for payload in payloads
        ),
        "recall_at_5": _mean_payload_value(answerable, "recall_at_5"),
        "recall_at_10": _mean_payload_value(answerable, "recall_at_10"),
        "mrr": _mean_payload_value(answerable, "mrr"),
        "ndcg_at_10": _mean_payload_value(answerable, "ndcg_at_10"),
        ...
        "forbidden_id_count": _sum_payload_int(payloads, "forbidden_id_count"),
        "no_result_pass_rate": _mean_bool_payload_value(
            payloads,
            "no_result_pass",
        ),
    }
...
def _mean_bool_payload_value(
    ...
    values = [
        payload[key] for payload in payloads if isinstance(payload.get(key), bool)
    ]
    if not values:
        return 0.0
...
def _case_has_context_expectations(case: object) -> bool:
    return bool(
        ...
        or getattr(case, "forbidden_ids", ())
        or getattr(case, "expect_no_results", False)
    )
