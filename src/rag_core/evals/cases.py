...
class EvalCase:
    ...
    def __init__(
        ...
        if expect_no_results and expected_ids:
            raise ValueError(
                "expect_no_results cases must not declare expected_ids"
            )
        if not expect_no_results and not expected_ids:
            raise ValueError(
                "expected_ids must not be empty unless expect_no_results is true"
            )
        ...
        object.__setattr__(self, "document_ids", tuple(document_ids))
        object.__setattr__(self, "forbidden_ids", tuple(forbidden_ids))
        object.__setattr__(self, "expect_no_results", expect_no_results)
...
def _load_case_row(row: object, *, path: Path, line_number: int) -> EvalCase:
    ...
    expect_no_results = optional_bool(
        row,
        "expect_no_results",
        path=path,
    ...
    expected_ids = (
        ...
        else expected_ids_tuple(row, path=path, line_number=line_number)
    )
    validate_unique_expected_ids(
        expected_ids,
        path=path,
    ...
    return EvalCase(
        ...
        collections=required_string_tuple(
            row,
            "collections",
            path=path,
            line_number=line_number,
        ...
        document_ids=optional_string_tuple(
            row,
            "document_ids",
            path=path,
            line_number=line_number,
        ),
        forbidden_ids=optional_string_tuple(
            row,
            "forbidden_ids",
            path=path,
            line_number=line_number,
