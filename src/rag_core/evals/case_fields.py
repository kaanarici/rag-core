...
def optional_bool(
    ...
    *,
    ...
    if not isinstance(value, bool):
        raise load_error(path, line_number, f"{field} must be a boolean")
    return value


def empty_allowed_string_tuple(
    row: dict[str, object],
    field: str,
    *,
    path: Path,
    ...
    if not isinstance(value, list):
        raise load_error(path, line_number, f"{field} must be a string array")
    items: list[str] = []
    for item in value:
        if not isinstance(item, str) or not item.strip():
            raise load_error(
...
def expected_ids_tuple(
    ...
    *,
    ...
    return required_string_tuple(row, "expected_ids", path=path, line_number=line_number)
