...
from rag_core.remote_ingest_call import remote_url_core_ingest_kwargs
...
async def ingest_remote_urls(
    ...
    *,
    ...
    records: list[RemoteUrlIngestSuccess | RemoteUrlIngestFailure | None] = [
        None for _ in plan.urls
    ]
    progress = RemoteUrlIngestProgress(event_sink=event_sink, plan=plan)
    if not plan.urls:
        return []
    ...
    async def ingest_url(position: int, item: RemoteUrlSourceItem) -> None:
        ...
        while True:
            async with url_iter_lock:
                try:
                    ...
                    return
            await ingest_url(position, item)
