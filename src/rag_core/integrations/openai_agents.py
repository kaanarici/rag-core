...
def build_retrieve_context_tool(
    ...
    *,
    ...
    expose_strategy_options: bool = True,
) -> Any:
    """Build an OpenAI Agents SDK function tool around ``Engine.context``.

    ``collection`` and optional ``namespace`` are bound when the tool is constructed.
    ...
    default_collections_tuple, default_document_ids = normalize_static_retrieval_scope(
        collections=resolve_collections_argument(
            collection=collection,
            collections=collections,
            caller="build_retrieve_context_tool",
        ),
    ...
    async def search_user_documents_scoped(
        query: str,
        limit: int | None = None,
        document_ids: list[str] | None = None,
        max_chars: int | None = None,
    ...
    tool_function = (
        ...
        if expose_strategy_options
        else search_user_documents_scoped
    )
    return decorate(tool_function)
