...
def _consumer_app() -> str:
    return textwrap.dedent(
        ...
        from rag_core import Document, RAGCore
        ...
        async def _rag_core_facade_smoke() -> dict[str, object]:
            rag = RAGCore(
            ...
            return {
                ...
                "rag_core_source_id": result.evidence[0].source_id,
            }


        async def _no_key_context_smoke(app_dir: Path) -> dict[str, object]:
            core = build_demo_core(store_collection="wheel_smoke_consumer")
            async with core:
        ...
        def _integration_import_smoke() -> dict[str, object]:
            ...
            class _RetrieveContextCore:
                async def context(self, **kwargs: object) -> object:
                    del kwargs
            ...
            class _McpCore:
                async def search(self, **kwargs: object) -> object:
                    del kwargs
                    raise AssertionError("MCP builder should fail before search")

                async def context(self, **kwargs: object) -> object:
                    del kwargs
                    raise AssertionError("MCP builder should fail before retrieval")

            try:
                integrations.build_mcp_server(
                    _McpCore(),
                    namespace="acme",
                    collections=["help-center"],
                )
                ...
                if "mcp" not in str(exc):
                    raise AssertionError(f"unexpected MCP error: {exc}") from exc
            else:
                raise AssertionError("expected MCP extra to be required")

            return {
                "integration_export_count": len(expected_exports),
                "integration_optional_extra_errors": ["langchain", "openai-agents", "mcp"],
            }
        ...
        async def _eval_smoke(app_dir: Path) -> dict[str, object]:
            ...
            async with build_demo_core(store_collection="wheel_smoke_eval") as core:
                ...
                results = await run_eval(
                    ...
                    query_plan=search_profile("fast", limit=5),
                )
            ...
            if (
                ...
                or metrics.get("recall_at_5") != 1.0
                or metrics.get("mrr") != 1.0
            ):
                raise AssertionError(f"expected wheel eval metrics to pass: {report}")
            return {
                "eval_case_count": report["case_count"],
                "eval_recall_at_5": metrics["recall_at_5"],
            }
        ...
        def _installed_cli_smoke(app_dir: Path) -> dict[str, object]:
            ...
            if not isinstance(hits, list) or not hits:
                raise AssertionError("expected installed search CLI hits")
            if search_payload.get("indexed_count", 0) < 1:
                raise AssertionError("expected installed search CLI to index files")
            ...
            if (
                ...
                raise AssertionError("expected installed eval metrics to pass")
        ...
        async def main() -> None:
            ...
            facade_smoke = await _rag_core_facade_smoke()
