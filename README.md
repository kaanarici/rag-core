...
`context()` returns prompt-ready context with citations; `search()` returns the raw
ranked hits.
...
rag-core adds files, archives, and URLs to an app-owned scope; searches with
capability-aware query plans; returns `SearchResult` hits or a `Context` of
