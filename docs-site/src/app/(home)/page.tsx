...
async with RAGCore(
    ...
    evidence = await rag.search("How can invoices be paid?")`;

const features = [
  {
    icon: Cpu,
    title: 'Scoped by construction',
    body: 'Bind tenant, index, and optional document scope before retrieval reaches an agent.',
  },
  {
    icon: Database,
    title: 'Scales up',
  ...
  },
  {
    ...
    body: 'Get source identity, locators, and deterministic duplicate collapsing.',
  },
  {
    icon: Blocks,
    title: 'Embeddable',
      ...
      <section className="mx-auto grid w-full max-w-6xl items-center gap-12 px-6 py-20 lg:grid-cols-2 lg:py-28">
        <div className="flex flex-col items-start">
          ...
          <p className="mt-5 max-w-md text-lg leading-relaxed text-fd-muted-foreground">
            rag-core is an embeddable engine for infrastructure your application
            already owns. Bind a scope, add documents, and give an agent a
            trustworthy retrieval tool.
          </p>
      ...
      <section className="mx-auto w-full max-w-6xl px-6 pb-20">
        <div className="max-w-2xl">
          <h2 className="text-2xl font-semibold tracking-tight">
            One facade from tests to production
          </h2>
          <p className="mt-3 leading-relaxed text-fd-muted-foreground">
            Use the local config for tests and onboarding. Production uses the
            same facade with an existing vector store and embedding provider.
          </p>
        </div>
