...
def test_vector_store_provider_order_uses_capability_specs() -> None:
    sources = {
        ...
        for path in (
            ...
            "src/rag_core/search/providers/pgvector_store.py",
...
def test_pgvector_adapter_imports_search_contract_owners_directly() -> None:
    ...
    pgvector_sources = {
        path: path.read_text(encoding="utf-8")
        for path in provider_root.glob("pgvector_*.py")
    }

    assert pgvector_sources
    for path, source in pgvector_sources.items():
        assert "from rag_core.search.types import" not in source, path
    assert "rag_core.search.filters" in pgvector_sources[
        provider_root / "pgvector_filters.py"
    ]
    assert "rag_core.search.provider_protocols" in pgvector_sources[
        provider_root / "pgvector_store.py"
    ]
    assert "rag_core.search.request_models" in pgvector_sources[
        provider_root / "pgvector_store.py"
    ]
    assert "rag_core.search.vector_models" in pgvector_sources[
        provider_root / "pgvector_payloads.py"
    ]
