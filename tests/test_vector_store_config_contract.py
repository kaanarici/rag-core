...
from rag_core.config import PgVectorStoreConfig, VectorStoreConfig
...
def test_vector_store_config_rejects_unknown_provider() -> None:
    ...
    assert str(exc_info.value) == (
        "VectorStoreConfig.provider must be one of: qdrant, pgvector, turbopuffer"
    )


def test_pgvector_provider_requires_dsn_at_config_boundary() -> None:
    with pytest.raises(ValueError) as exc_info:
        VectorStoreConfig(provider="pgvector")
...
def test_pgvector_provider_accepts_valid_dsn() -> None:
    config = VectorStoreConfig(
        provider="pgvector",
        pgvector=PgVectorStoreConfig(dsn="postgresql://localhost/rag_core"),
    )

    assert config.provider == "pgvector"
    assert config.pgvector.dsn == "postgresql://localhost/rag_core"
