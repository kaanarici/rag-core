...
def _build_real_store(config: Config, embedding: DemoEmbeddingProvider) -> VectorStore:
    ...
    return create_configured_vector_store(
        ...
        sparse_enabled=config.ingest.build_sparse_index,
...
) -> tuple[Engine, _FailingVectorStore]:
    """Construct a demo-equivalent ``Engine`` whose single real Qdrant
...
async def _search_hits(core: Engine, query: str, *, document_id: str | None = None) -> list[SearchResult]:
...
async def _sidecar_hits(
    ...
    return await sidecar.search(
        SearchSidecarQuery(query=query, namespace="recovery", collections=["docs"])
    )
