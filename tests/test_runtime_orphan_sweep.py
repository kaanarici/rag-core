...
def test_job_retention_default_keeps_terminal_rows(tmp_path: Path) -> None:
    ...
    completed = seed.create(path="/tmp/a.md", namespace="acme", collection="public")
    seed.update(
        completed.job_id,
...
def test_job_retention_expires_terminal_rows_after_reopen(
    ...
    db_path = tmp_path / "jobs.sqlite3"
    seed = IngestJobStore(db_path)
    old = seed.create(path="/tmp/old.md", namespace="acme", collection="public")
    seed.update(old.job_id, status=INGEST_JOB_STATUS_COMPLETED)
    fresh = seed.create(path="/tmp/fresh.md", namespace="acme", collection="public")
...
def test_job_retention_never_prunes_pending_or_running(
    ...
    db_path = tmp_path / "jobs.sqlite3"
    seed = IngestJobStore(db_path)
    pending = seed.create(path="/tmp/pending.md", namespace="acme", collection="public")
    running = seed.create(path="/tmp/running.md", namespace="acme", collection="public")
    seed.update(running.job_id, status=INGEST_JOB_STATUS_RUNNING)
    completed = seed.create(path="/tmp/done.md", namespace="acme", collection="public")
...
def test_job_retention_trims_oldest_terminal_rows_on_terminal_write(
    ...
    first = store.create(path="/tmp/a.md", namespace="acme", collection="public")
    store.update(first.job_id, status=INGEST_JOB_STATUS_COMPLETED)
    second = store.create(path="/tmp/b.md", namespace="acme", collection="public")
    store.update(second.job_id, status=INGEST_JOB_STATUS_COMPLETED)
    third = store.create(path="/tmp/c.md", namespace="acme", collection="public")
...
def test_reconcile_orphaned_jobs_flips_running_and_pending(tmp_path: Path) -> None:
    ...
    pending = store.create(path="/tmp/a.md", namespace="acme", collection="public")
    running = store.create(path="/tmp/b.md", namespace="acme", collection="public")
    store.update(running.job_id, status=INGEST_JOB_STATUS_RUNNING)
    completed = store.create(path="/tmp/c.md", namespace="acme", collection="public")
    store.update(
        completed.job_id,
        status=INGEST_JOB_STATUS_COMPLETED,
        result={"document_id": "doc-c"},
    )
    pre_failed = store.create(path="/tmp/d.md", namespace="acme", collection="public")
    store.update(
        pre_failed.job_id,
...
def test_reconcile_orphaned_jobs_is_no_op_when_clean(tmp_path: Path) -> None:
    store = IngestJobStore(tmp_path / "jobs.sqlite3")
    done = store.create(path="/tmp/a.md", namespace="acme", collection="public")
    store.update(
        done.job_id,
        status=INGEST_JOB_STATUS_COMPLETED,
        result={"document_id": "doc-a"},
...
def test_reconcile_orphaned_jobs_custom_reason_stamped(tmp_path: Path) -> None:
    store = IngestJobStore(tmp_path / "jobs.sqlite3")
    running = store.create(path="/tmp/a.md", namespace="acme", collection="public")
    store.update(running.job_id, status=INGEST_JOB_STATUS_RUNNING)
