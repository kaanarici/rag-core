...
def test_fetch_security_policy_allowed_hosts_empty_denies_all_outbound() -> None:
    policy = FetchSecurityPolicy(allowed_hosts=())

    with pytest.raises(ValueError, match="not in allowed_hosts"):
        validate_fetch_url("https://example.com/docs", policy=policy)
    with pytest.raises(ValueError, match="not in allowed_hosts"):
        validate_fetch_url("https://docs.example.com/", policy=policy)


def test_fetch_security_policy_allowed_hosts_exact_match() -> None:
    policy = FetchSecurityPolicy(allowed_hosts=("docs.example.com",))

    assert (
        validate_fetch_url("https://docs.example.com/guide", policy=policy).host
        == "docs.example.com"
    )

    with pytest.raises(ValueError, match="not in allowed_hosts"):
        validate_fetch_url("https://api.example.com/guide", policy=policy)
    with pytest.raises(ValueError, match="not in allowed_hosts"):
        validate_fetch_url("https://other.example.com/guide", policy=policy)


def test_fetch_security_policy_allowed_hosts_wildcard_matches_subdomains() -> None:
    policy = FetchSecurityPolicy(allowed_hosts=("*.example.com",))

    assert (
        validate_fetch_url("https://docs.example.com/x", policy=policy).host
        == "docs.example.com"
    )
    assert (
        validate_fetch_url("https://a.b.example.com/x", policy=policy).host
        == "a.b.example.com"
    )
    assert (
        validate_fetch_url("https://example.com/x", policy=policy).host
        == "example.com"
    )

    with pytest.raises(ValueError, match="not in allowed_hosts"):
        validate_fetch_url("https://example.net/x", policy=policy)
    with pytest.raises(ValueError, match="not in allowed_hosts"):
        validate_fetch_url("https://evil-example.com/x", policy=policy)


def test_fetch_security_policy_allowed_hosts_normalizes_case_and_idna() -> None:
    policy = FetchSecurityPolicy(allowed_hosts=("DOCS.Example.COM",))
    ...
    assert policy.allowed_hosts == ("docs.example.com",)


def test_fetch_security_policy_allowed_hosts_rejects_malformed_patterns() -> None:
    with pytest.raises(ValueError, match="non-empty"):
        FetchSecurityPolicy(allowed_hosts=("",))
    with pytest.raises(ValueError, match="leading"):
        FetchSecurityPolicy(allowed_hosts=("foo*bar.com",))
    with pytest.raises(ValueError, match="wildcard"):
        FetchSecurityPolicy(allowed_hosts=("*.",))


def test_fetch_security_policy_allowed_hosts_enforced_on_redirect_chain() -> None:
    policy = FetchSecurityPolicy(allowed_hosts=("docs.example.com",))

    # Initial hop matches, redirect-target does not.
    with pytest.raises(ValueError, match="not in allowed_hosts"):
        validate_fetch_redirects(
            [
    ...
    # Both hops match.
    chain = validate_fetch_redirects(
        [
            "https://docs.example.com/start",
