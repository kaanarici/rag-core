...
from rag_core.search import Context
...
def _prompt_ranks(text: str) -> list[int]:
    return [
        int(match) for match in re.findall(r"^\[S(\d+)\]", text, flags=re.MULTILINE)
    ]
...
def test_context_pack_context_order_keeps_app_payload_rank_order_byte_identical() -> (
    None
):
    pack = _context_order_pack(5)
    ...
    app_extrema_payload = {
        k: v for k, v in extrema_payload.items() if k != "context_text"
    }
...
def test_context_pack_collapses_exact_duplicate_files_but_keeps_near_duplicates() -> (
    None
):
    exact_hash = "sha256:exact"
    near_hash = "sha256:near"
    pack = build_context_pack(
        [
            make_search_result(
                id="exact-a",
                text="same exact content",
                document_id="doc-a",
                document_key="a.md",
            ...
            make_search_result(
                id="near",
                text="same nearby content",
                document_id="doc-near",
                document_key="near.md",
                content_sha256=near_hash,
                chunk_index=0,
            ),
        ],
        query="duplicate control",
    )

    assert [snippet.source.content_sha256 for snippet in pack.snippets] == [
        exact_hash,
        near_hash,
    ]
...
def test_context_pack_keeps_exact_content_across_scopes_and_prepared_texts() -> None:
    exact_hash = "sha256:exact"
    pack = build_context_pack(
        [
            make_search_result(
                id="course-a-original",
                text="same source, course A preparation",
                namespace="school",
                collection="course-a",
                document_id="doc-a",
                content_sha256=exact_hash,
            ...
            make_search_result(
                id="course-a-contextualized",
                text="same source, distinct contextualized preparation",
                namespace="school",
                collection="course-a",
                document_id="doc-c",
                content_sha256=exact_hash,
        ...
        query="scope-aware duplicate control",
    )

    assert [snippet.text for snippet in pack.snippets] == [
        "same source, course A preparation",
        "same source, course A preparation",
        "same source, distinct contextualized preparation",
        "same source, course A preparation",
    ]
