...
from rag_core.cli.inputs import cli_error_message, parse_metadata_fields
...
from rag_core.manifest.persistence import ManifestSource, write_entry
...
class _SiblingAbortLocalIngestCore(_FakeLocalIngestCore):
    def __init__(self, *, fail_path: Path) -> None:
        super().__init__()
        self.fail_path = fail_path
        self.active = 0
...
def test_run_local_search_uses_the_dense_default_profile(tmp_path: Path) -> None:
    docs = tmp_path / "docs"
    docs.mkdir()
    (docs / "billing.md").write_text("Invoices are monthly.", encoding="utf-8")

    class CapturingCore(_FakeLocalSearchCore):
        query_plan: object | None = None

        async def _search_hits(self, **kwargs: Any) -> list[SearchResult]:
            self.query_plan = kwargs["query_plan"]
    ...
    assert DEFAULT_SEARCH_PROFILE == "fast"
    assert core.query_plan == search_profile("fast", limit=5)
...
def test_run_local_search_uses_single_file_document_key_and_collection(tmp_path) -> None:
    ...
    assert result.to_payload()["collection"] == "guide"
    assert core.ingest_calls == [
...
def test_run_local_search_indexes_files_reports_failed_and_returns_hits(
    ...
    assert core.search_calls[0].collections == ["docs"]
...
def test_build_local_ingest_plan_expands_paths_with_portable_keys_and_hashes(
    ...
    plan = build_local_ingest_plan(
        LocalIngestRequest(path=docs, namespace="acme", collection="help")
    )
...
def test_build_local_ingest_plan_uses_file_and_glob_key_roots(tmp_path) -> None:
    ...
    file_plan = build_local_ingest_plan(
        LocalIngestRequest(path=file_path, namespace="acme", collection="help")
    )
...
def test_build_local_ingest_plan_rejects_hardlinked_file_path(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="does not allow multi-link"):
        build_local_ingest_plan(
            LocalIngestRequest(path=alias, namespace="acme", collection="help")
        )
...
def test_run_local_ingest_reports_partial_failures_and_closes_core(tmp_path) -> None:
    ...
    assert (started[0].namespace, started[0].collection, started[0].planned_count) == (
        "acme",
        "help",
        2,
    )
...
async def _run_local_ingest_respects_max_concurrency(tmp_path) -> None:
    ...
    try:
        ...
        await task
        raise
    assert sorted(path.name for path in core.started) == ["a.md", "b.md"]
...
def test_run_local_ingest_single_file_reuses_initial_read_for_ingest_bytes(
    ...
    import rag_core.ingest.local.api as local_ingest_module
...
def test_local_ingest_source_read_failures_are_unknown_manifest_status(
    ...
    import rag_core.ingest.local.api as local_ingest_module
    import rag_core.ingest.sources.local as sources_module
...
def test_run_local_ingest_drains_sibling_workers_before_close(
    tmp_path: Path,
) -> None:
    docs = tmp_path / "docs"
    docs.mkdir()
    ...
    second = docs / "b.md"
    second.write_text("beta", encoding="utf-8")
    core = _SiblingAbortLocalIngestCore(fail_path=first)
...
def test_folder_ingest_reads_each_file_once(
    tmp_path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import rag_core.ingest.local.runner as runner_module
    import rag_core.ingest.sources.local as sources_module
    ...
    original_read = runner_module.read_file_bytes
    original_hash = sources_module.file_content_sha256
    ...
    monkeypatch.setattr(runner_module, "read_file_bytes", counting_read)
    monkeypatch.setattr(sources_module, "file_content_sha256", counting_hash)
    ...
    # Discovery: exactly one hash call per file.
    assert sorted(hash_paths) == sorted([file_a, file_b])
    # Runner read: exactly one read call per file. No second read inside ingest_file.
    assert sorted(read_paths) == sorted([file_a, file_b])


def test_bytes_ingest_adapter_prefers_fresh_pre_read_bytes(tmp_path: Path) -> None:
    """The single-file adapter must index the runner's fresh ingest-time bytes,
    not the stale plan-time bytes, so a file changed between discovery and
    ingest lands its current content. Falls back to plan-time bytes only when
    the runner did not pre-read.
    """
    from rag_core.ingest.local.api import _BytesIngestAdapter
    ...
    class _RecordingCore:
        async def add_bytes(
            self, *, file_bytes: bytes, **_: object
        ) -> IngestedDocument:
            received.append(file_bytes)
            return IngestedDocument(
                document_id="doc-x",
                collection="c",
                namespace="n",
    ...
    adapter = _BytesIngestAdapter(
        core=cast(Any, _RecordingCore()),
        file_bytes=b"stale plan-time bytes",
        expected_path=path,
    )
    ...
    asyncio.run(
        adapter.add_file(
            ...
            collection="c",
            document_key="a.txt",
            pre_read_bytes=b"fresh ingest-time bytes",
        )
    )
    assert received == [b"fresh ingest-time bytes"]

    received.clear()
    asyncio.run(
        adapter.add_file(path, namespace="n", collection="c", document_key="a.txt")
    )
    assert received == [b"stale plan-time bytes"]
