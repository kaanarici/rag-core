...
def test_run_eval_concurrent_path_preserves_input_order() -> None:
    ...
    results = asyncio.run(
        run_eval(cast(Engine, _DelayedCore()), cases, max_concurrency=3)
    )

    assert [result.case.query for result in results] == [case.query for case in cases]
...
def test_run_eval_concurrent_path_isolates_case_failures() -> None:
    ...
    results = asyncio.run(
        run_eval(cast(Engine, _MixedCore()), cases, max_concurrency=2)
    )
    ...
    async def go() -> list[EvalResult]:
        ...
        try:
            ...
            for doc_id, body in (
                (
                    "billing",
                    "Billing happens monthly. Customers receive a billing email.",
                ),
                (
                    "shipping",
                    "Shipping rates depend on weight. International shipping uses customs.",
                ),
            ):
...
def test_run_eval_passes_query_plan_to_core_search() -> None:
    async def go() -> RecordingVectorStore:
        store = RecordingVectorStore(
            search_results=[
                make_search_result(document_id="billing", collection="docs")
            ]
        )
        core = Engine(
            make_test_config(
...
def test_run_eval_enforces_document_scope_and_forbidden_ids() -> None:
    ...
    class _ScopedCore:
        async def search(self, **kwargs: object) -> list[object]:
            ...
            return [
                make_search_result(
                    document_id="internal-forbidden",
                    document_key="forbidden-source",
                )
            ]
    ...
    case = EvalCase(
        ...
        namespace="rt",
        collections=("docs",),
        document_ids=("internal-allowed",),
        expected_ids=("allowed-source",),
        forbidden_ids=("forbidden-source",),
    )

    [result] = asyncio.run(run_eval(cast(Engine, _ScopedCore()), [case]))

    assert captured["document_ids"] == ["internal-allowed"]
    assert result.retrieved_ids == ("forbidden-source",)
    assert result.forbidden_id_count == 1
    assert result.prompt_safety_pass is False


def test_run_eval_reports_empty_scope_without_ranking_credit() -> None:
    class _EmptyCore:
        async def search(self, **_: object) -> list[object]:
            return []
    ...
    [result] = asyncio.run(run_eval(cast(Engine, _EmptyCore()), [case]))
...
def test_eval_report_excludes_no_result_controls_from_ranking_metrics() -> None:
    answerable = EvalResult(
        case=EvalCase(
            query="answerable",
            namespace="rt",
            collections=("docs",),
    ...
    report = eval_report([answerable, empty_scope])
    metrics = cast(dict[str, object], report["metrics"])

    assert metrics["answerable_case_count"] == 1
    assert metrics["no_result_case_count"] == 1
    assert metrics["mrr"] == 0.5
    assert metrics["ndcg_at_10"] == pytest.approx(0.6309297535714575)
    assert metrics["no_result_pass_rate"] == 1.0
...
def test_run_eval_can_measure_rerank_improvement() -> None:
    async def go() -> tuple[list[EvalResult], list[EvalResult], FakeReranker]:
        ...
        reranker = FakeReranker(
            results=[
                RerankResult(
                    index=1, score=0.99, text="billing policy and invoice schedule"
                ),
