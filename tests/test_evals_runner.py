...
def test_eval_case_is_frozen() -> None:
    case = EvalCase(query="q", namespace="ns", corpus_ids=("c",), expected_ids=("a",))
...
def test_run_eval_rejects_non_positive_max_concurrency() -> None:
    with pytest.raises(ValueError, match="max_concurrency must be positive"):
        asyncio.run(run_eval(cast(RAGCore, object()), [], max_concurrency=0))
...
def test_run_eval_default_matches_explicit_sequential_path(
    ...
    default_results = asyncio.run(run_eval(cast(RAGCore, _StableCore()), cases))
    ticker.current = 0.0
    explicit_results = asyncio.run(
        run_eval(cast(RAGCore, _StableCore()), cases, max_concurrency=1)
    )
...
def test_run_eval_concurrent_path_preserves_input_order() -> None:
    ...
    results = asyncio.run(run_eval(cast(RAGCore, _DelayedCore()), cases, max_concurrency=3))
...
def test_run_eval_concurrent_path_respects_max_concurrency() -> None:
    ...
    results = asyncio.run(run_eval(cast(RAGCore, core), cases, max_concurrency=2))
...
def test_run_eval_concurrent_path_isolates_case_failures() -> None:
    ...
    results = asyncio.run(run_eval(cast(RAGCore, _MixedCore()), cases, max_concurrency=2))
