...
def test_turbopuffer_docs_name_actual_adapter_proof() -> None:
    ...
    for path in (
        ...
        "tests/test_turbopuffer_contextual_bm25.py",
...
def test_custom_provider_table_matches_runtime_injection_surface() -> None:
    ...
    assert (
        "Qdrant/pgvector/TurboPuffer via `RAGCoreConfig.vector_store`"
    ...
    assert "RAGCoreConfig.contextualizer" in rows["Chunk contextualizers"]["runtime_selection"]
