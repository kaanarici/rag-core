...
- ``Engine`` rejects a vector store missing required capabilities.
...
from tests.support.pgvector_fake import PgVectorFakePool
...
class _MinimalVectorStore:
    ...
    async def get_chunks_by_index(
        ...
        *,
        ...
        raise NotImplementedError(
            "_MinimalVectorStore does not implement get_chunks_by_index"
        )


def test_qdrant_store_defaults_to_dense_capability_surface() -> None:
    ...
    try:
        ...
        assert store.capabilities.query_plan.dense is True
        assert store.capabilities.query_plan.sparse is False
        assert store.capabilities.dense_vector_dimensions == 4
    finally:
...
def test_pgvector_store_declares_adapter_tested_capability_surface() -> None:
    store = PgVectorVectorStore(
        table_name="docs",
        dense_dimensions=4,
        pool=PgVectorFakePool(),
    )
    assert isinstance(store, VectorStore)
    assert store.capabilities == PGVECTOR_VECTOR_STORE_CAPABILITY_SPEC.to_store_capabilities(
        dense_vector_dimensions=4,
    )
...
def test_minimal_vector_store_is_vector_store_with_empty_capabilities() -> None:
    ...
    assert store.capabilities.chunk_index_lookup is False
...
def test_rag_core_round_trip_against_memory_vector_store() -> None:
    ...
    Qdrant- or TurboPuffer-specific is needed to wire ``Engine``.
    """
    ...
    async def _run() -> None:
        ...
        assert hits[0].collection == "corpus-1"
...
def test_vector_stores_registry_lists_known_factories() -> None:
    ...
    assert {"memory", "pgvector", "qdrant", "turbopuffer"}.issubset(set(names))
