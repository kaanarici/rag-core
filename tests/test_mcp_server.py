...
from rag_core.search.context_pack_models import ContextPack
...
class _FailingCore:
    ...
    async def retrieve_context(self, **_: Any) -> ContextPack:
        raise RuntimeError(self.message)
...
def test_mcp_server_lists_read_only_tools_with_scope_bound_schemas() -> None:
    async def scenario() -> None:
        core = _FakeCore([])
        server = build_mcp_server(core, namespace="acme", corpus_ids=["help"])
        ...
        async def check(session: ClientSession) -> None:
            ...
            for tool in tools.values():
                ...
                assert "corpus_ids" not in schema["properties"]
...
def test_mcp_search_and_retrieve_context_bind_scope_and_return_structured_content() -> None:
    async def scenario() -> None:
        ...
        assert search_call["corpus_ids"] == ["help"]
...
def test_mcp_tools_run_against_demo_core_with_memory_qdrant() -> None:
    async def scenario() -> None:
        core = build_demo_core(collection=f"mcp_server_{uuid4().hex}")
        try:
