...
from rag_core.manifest.reconciliation import ManifestSource, reconcile_entries
...
ROOT_REMOVED_EXPORTS = ("index", "Index", "Engine", "Context", "SearchResult")
...
def test_shipped_public_stubs_match_runtime_exports() -> None:
    ...
    for module_name, stub_path in stubbed_modules.items():
        ...
        if module_name == "rag_core":
            ...
            assert set(ROOT_ADVANCED_DOOR_NAMES).isdisjoint(stub_names)
        else:
...
def test_ingest_results_aggregator_exposes_local_and_remote_outcomes() -> None:
    import rag_core.ingest.results as ingest_results
    from rag_core.ingest.local.models import LocalIngestResult as _LocalResult
...
def test_root_day_one_surface_is_small_and_advanced_shapes_stay_owner_imports() -> None:
    day_one_types = (
        RAGCore,
        Config,
        Document,
    ...
    for name in ROOT_REMOVED_EXPORTS:
        assert not hasattr(rag_core, name)
...
def test_root_surface_has_no_deprecated_aliases() -> None:
    for old_name in ("Rag", "RAGCoreConfig", "ContextPack", "SourceReference"):
        assert not hasattr(rag_core, old_name)


def test_engine_facade_methods_are_curated() -> None:
    beta_core_methods = {
        "add",
        "add_archive",
        "add_bytes",
...
def test_stability_docs_name_the_actual_rag_core_ingest_shape() -> None:
    ...
    assert "add bytes, files, folders, ZIP archives, URLs, or URL lists" in stability
    assert "`ingest_files`" not in stability
    assert "files/folders" not in stability
    assert "`Engine.ingest_folder`" not in stability
...
def test_context_pack_citation_primitives_are_public_imports() -> None:
    ...
    assert search_module.Citation is SearchCitation
...
def test_integration_surfaces_type_against_engine_owner_module() -> None:
    for path in (
        ...
        assert "from rag_core.core import Engine" in source
        assert "from rag_core import Engine" not in source


def test_eval_runner_types_against_owner_modules() -> None:
...
def test_root_package_does_not_reexport_advanced_search_types() -> None:
    ...
    for source in (root, stub):
        assert "from .search import Context" not in source
        assert "from .search import SearchResult" not in source
        assert ".search.context_pack import" not in source
        assert ".search.types import SearchResult" not in source
    assert '"SearchResult": ("rag_core.search", "SearchResult")' not in root
    assert '"Context": ("rag_core.search", "Context")' not in root
...
def test_integration_protocols_reuse_tool_contract_context_protocol() -> None:
    ...
    assert "SupportsContextPromptPayload" in protocols
    assert not context_text.exists()
    assert "SupportsContextPromptPayload" in langchain_payloads
    assert "pack: SupportsContextPromptPayload" in langchain_stub
    assert "class SupportsContextPackPayload" not in tool_contracts
    assert "class SupportsContextPromptPayload" in tool_contracts
    assert (
...
def test_provider_exports_are_curated() -> None:
    ...
    # The trailing ``namespace`` / ``collection`` / ``document_id`` fields
...
def test_stability_docs_do_not_overclaim_provider_root_vector_store_exports() -> None:
    stability = Path("docs-site/content/docs/stability.mdx").read_text(encoding="utf-8")
    normalized = " ".join(stability.split())

    # Only the default Qdrant adapter is named at the provider root; optional and
    # utility vector stores are reached through config or the registry, not
    # advertised as additional provider-root adapter exports.
    assert "the default Qdrant vector-store adapter" in stability
    assert (
        "Optional and utility vector stores are selected through config or the registry"
        in normalized
    )
...
def test_rag_core_search_returns_public_search_result_with_payload() -> None:
    ...
    assert query.collections == ["corpus-a"]
