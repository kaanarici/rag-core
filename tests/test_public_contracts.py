...
def test_shipped_public_stubs_match_runtime_exports() -> None:
    ...
    for module_name, stub_path in stubbed_modules.items():
        ...
        elif module_name == "rag_core.search":
            assert set(module.__all__).issubset(stub_names)
        else:
            assert stub_names == set(module.__all__)
...
def test_root_surface_has_no_deprecated_aliases() -> None:
    for old_name in ("Rag", "RAGCore", "RAGCoreConfig", "ContextPack", "SourceReference"):
...
def test_stability_docs_name_the_actual_rag_core_ingest_shape() -> None:
    ...
    assert "`Document`" in stability
    assert "`Engine`" in stability
    assert "`Scope`" in stability
    assert "`ingest_files`" not in stability
    assert "files/folders" not in stability
    assert "`Engine.ingest_folder`" not in stability
    # Public docs describe the product contract, not an internal symbol owner.
    assert "provider_protocols" not in stability
...
def test_integration_surfaces_type_against_root_rag_core() -> None:
    for path in (
        ...
        assert "from rag_core import Engine" in source
...
def test_stability_docs_keep_provider_implementations_in_focused_modules() -> None:
    stability = Path("docs-site/content/docs/stability.mdx").read_text(encoding="utf-8")

    assert "rag_core.search.providers" in stability
    assert '"GoogleEmbedding"' not in stability
    assert '"PgVector"' not in stability
