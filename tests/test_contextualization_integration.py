...
def _build_core(
    *,
    ...
    event_sink: Any = None,
) -> tuple[Engine, FakeEmbeddingProvider, RecordingVectorStore]:
...
def test_disabled_contextualizer_config_keeps_contextualizer_absent(
    ...
    assert '"contextualizer_id"' not in str(runtime["pipeline_version"])
    assert '"contextualizer_chunk_cap"' not in str(runtime["pipeline_version"])
    for chunk in prepared.chunks:
        assert chunk.embedding_text == chunk.text
...
def test_injected_contextualizer_overrides_contextualizer_config(
    ...
    processing_version = str(runtime["pipeline_version"])
...
def test_contextualizer_chunk_cap_participates_in_processing_fingerprint() -> None:
    ...
    try:
        version_a = str(core_a.describe_runtime()["pipeline_version"])
        version_b = str(core_b.describe_runtime()["pipeline_version"])
    finally:
        asyncio.run(core_a.close())
        asyncio.run(core_b.close())
...
def test_pre_wrapped_capped_contextualizer_enforces_cap_and_fingerprints_cap() -> None:
    ...
    try:
        version = str(core.describe_runtime()["pipeline_version"])
        prepared = asyncio.run(
            core.prepare_bytes(
                file_bytes=_DOCUMENT.encode("utf-8"),
                filename="doc.md",
