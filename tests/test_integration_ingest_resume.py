...
from rag_core.remote_ingest import run_remote_url_ingest_with_core
...
class _CrashAfterLocalCore:
    ...
    async def ingest_file(
        ...
        *,
        ...
        ingested = await self.core.ingest_file(
            file_path,
            namespace=namespace,
            corpus_id=corpus_id,
...
class _CrashAfterRemoteCore:
    ...
    async def ingest_url(
        ...
        *,
        ...
        ingested = await self.core.ingest_url(
            url,
            namespace=namespace,
            corpus_id=corpus_id,
            metadata=metadata,
...
def _build_core(
    manifest_dir: Path,
) -> tuple[RAGCore, _CountingDemoEmbeddingProvider, _CountingDemoSparseEmbedder]:
    dense = _CountingDemoEmbeddingProvider()
    sparse = _CountingDemoSparseEmbedder()
    config = RAGCoreConfig(
        qdrant=QdrantConfig(
            location=":memory:",
            collection=f"rag_core_resume_{uuid.uuid4().hex}",
    ...
    return (
        RAGCore(config, embedding_provider=dense, sparse_embedder=sparse),
        dense,
        sparse,
    )
...
def _assert_local_manifest_complete(manifest_dir: Path, docs: Path) -> None:
    entries = read_entries(manifest_dir, _NAMESPACE, _CORPUS_ID)
    expected = {
...
def _assert_manifested_success_count(manifest_dir: Path) -> None:
    assert len(read_entries(manifest_dir, _NAMESPACE, _CORPUS_ID)) == 2
