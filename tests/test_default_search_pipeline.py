...
from rag_core.retrieval_channels import SPARSE_RETRIEVAL_CHANNEL
...
def test_custom_pipeline_overrides_default() -> None:
    async def _run() -> None:
        ...
        results = await pipeline_runner.search(
            SearchRequest(query="query", collections=["corpus-1"], namespace="space-1")
        )
...
def test_sidecar_can_be_skipped_per_request_and_failures_are_isolated() -> None:
    async def _run() -> None:
        ...
        results = await with_failure.search(
            SearchRequest(query="q", collections=["c"], namespace="n")
        )
...
def test_query_plan_final_limit_mismatch_canonicalizes_request_limit() -> None:
    async def _run() -> None:
        events = EventBuffer()
        sidecar = FakeSearchSidecar(
            results=[make_search_result(id="doc-sidecar", namespace="n", collection="c")]
        )
        store = RecordingVectorStore(
            search_results=[
                make_search_result(id="doc-store", namespace="n", collection="c")
            ]
        )
        pipeline_runner = SearchPipelineRunner(
            embedding_provider=FakeEmbeddingProvider(),
...
def test_custom_pipeline_search_planned_reflects_transform_injected_plan() -> None:
    async def _run() -> None:
        ...
        await pipeline_runner.search(SearchRequest(query="q", collections=["c"], namespace="n"))
...
def test_default_pipeline_runs_rerank_when_requested_and_skips_when_not() -> None:
    async def _run() -> None:
        ...
        reranked = await pipeline_runner.search(
            SearchRequest(
                query="q", collections=["c"], namespace="n", rerank=True
            )
        )
        ...
        skipped = await pipeline_runner.search(
            SearchRequest(
                query="q", collections=["c"], namespace="n", rerank=False
            )
        )
...
def test_default_pipeline_does_not_report_real_rerank_when_no_provider_configured() -> None:
    async def _run() -> None:
        ...
        results = await pipeline_runner.search(
            SearchRequest(query="q", collections=["c"], namespace="n", rerank=True)
        )
...
def test_default_pipeline_preserves_budgeted_rerank_order_through_sidecar_merge() -> None:
    async def _run() -> None:
        hits = [
            make_search_result(id="a", text="alpha", score=0.95, namespace="n", collection="c"),
            make_search_result(id="b", text="beta", score=0.7, namespace="n", collection="c"),
            make_search_result(id="c", text="gamma", score=0.6, namespace="n", collection="c"),
        ]
        reranker = FakeReranker(
            results=[
        ...
        sidecar = FakeSearchSidecar(
            results=[make_search_result(id="side", score=1.0, namespace="n", collection="c")]
        )
        pipeline_runner = _pipeline_runner(
            store=RecordingVectorStore(search_results=hits),
            reranker=reranker,
...
def test_sidecar_hit_can_displace_weaker_vector_hit_when_limit_is_full() -> None:
    async def _run() -> None:
        sidecar = FakeSearchSidecar(
            results=[make_search_result(id="exact", score=1.0, namespace="n", collection="c")]
        )
        store = RecordingVectorStore(
            search_results=[
                make_search_result(id="vec-a", score=0.9, namespace="n", collection="c"),
                make_search_result(id="vec-b", score=0.8, namespace="n", collection="c"),
            ]
        )
        pipeline_runner = _pipeline_runner(store=store, sidecar=sidecar)

        results = await pipeline_runner.search(
            SearchRequest(query="q", collections=["c"], namespace="n", limit=2)
        )
...
def test_cancelled_search_cancels_sibling_embedding_task() -> None:
    """Cancelling a search must cancel the sibling query-embedding task.

    Regression: the cleanup used to live under ``except Exception``, so a
    ``CancelledError`` raised by the awaited dense task skipped it and left the
    sparse embedding task running provider work after the caller was gone.
    """

    async def _run() -> None:
        dense_entered = asyncio.Event()
        sparse_entered = threading.Event()
        release_sparse = threading.Event()

        class BlockingDenseProvider(FakeEmbeddingProvider):
            async def embed_query(self, query: str) -> list[float]:
                dense_entered.set()
                await asyncio.Event().wait()
                raise AssertionError("dense embed should never complete")

        class BlockingSparseEmbedder(FakeSparseEmbedder):
            def embed_query_multi(self, query: str):
                sparse_entered.set()
                release_sparse.wait(timeout=5.0)
                return super().embed_query_multi(query)

        events = EventBuffer()
        ctx = PipelineContext(
            embedding_provider=BlockingDenseProvider(),
            sparse_embedder=BlockingSparseEmbedder(),
            vector_store=RecordingVectorStore(
                search_results=[make_search_result(id="vec")]
            ),
            event_sink=events,
        )
        query = PipelineQuery(query="q", namespace="n", collections=["c"])

        retrieve_task = asyncio.create_task(HybridRetrieve().retrieve(query, ctx))
        await dense_entered.wait()
        await asyncio.to_thread(sparse_entered.wait, 5.0)
        assert sparse_entered.is_set()

        retrieve_task.cancel()
        with pytest.raises(asyncio.CancelledError):
            await retrieve_task

        # Let the orphaned sparse thread (if any) run to completion: with the
        # bug the un-cancelled sparse task would resume and emit a completion.
        release_sparse.set()
        for _ in range(50):
        ...
        sparse_completed = [
            event
            for event in events.events
            if isinstance(event, EmbedCompleted)
