...
def test_ingest_then_search_ranks_relevant_document_above_decoys() -> None:
    async def go() -> None:
        async with build_demo_core(store_collection=f"golden_{uuid.uuid4().hex}") as core:
            docs = {
...
def test_golden_path_uses_full_integration_corpus() -> None:
    async def go() -> None:
        async with build_demo_core(store_collection=f"golden10_{uuid.uuid4().hex}") as core:
