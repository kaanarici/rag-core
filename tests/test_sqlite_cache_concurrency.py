...
- ``test_corpus_policy_cache_disabled_swaps_in_no_cache``: contract. The
...
def test_build_cache_path_sanitizes_unsafe_segments(tmp_path: Path) -> None:
    ...
    # let a malicious corpus_id escape the base directory.
...
def test_corpus_policy_cache_disabled_default_false() -> None:
    from rag_core.search.policy import CorpusPolicy

    assert CorpusPolicy().cache_disabled is False


def test_corpus_policy_cache_disabled_swaps_to_no_cache(tmp_path: Path) -> None:
