...
def test_html_document_auto_chunks_as_markdown_after_conversion() -> None:
    ...
    assert chunks
    assert {chunk.chunking_strategy for chunk in chunks} == {MARKDOWN_CHUNKING_STRATEGY}
    assert all("line_start" not in chunk.metadata for chunk in chunks)
...
def test_prepared_chunks_preserve_router_positions_and_strategy() -> None:
    ...
    assert p.token_count == estimate_token_count(r.text)
