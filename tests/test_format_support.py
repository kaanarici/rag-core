...
from rag_core.local_sources import is_supported_local_file
...
def test_format_support_docs_cover_matrix() -> None:
    ...
    assert 'Converters emit parser metadata through `metadata["parser"]`' in docs
    assert "public trace and payload contract names" in docs
    assert "registry entry" in docs
