...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("report.pdf.txt", "application/octet-stream", False),
    ],
)
def test_pdf_identity_uses_mime_parameters_or_final_extension(
    filename: str,
    mime_type: str,
    expected: bool,
) -> None:
    assert (
        is_registered_pdf_document(filename=filename, mime_type=mime_type) is expected
    )
...
def test_local_ingest_support_matches_format_matrix() -> None:
    for extension in EXTENSION_MAP:
        ...
        assert support is not None
        assert (
            is_supported_local_file(Path(f"sample{extension}")) is support.local_ingest
        )
...
def test_binary_office_extensions_are_not_mapped() -> None:
    ...
    assert all(
        mime_type not in MIME_TYPE_MAP for mime_type in UNSUPPORTED_BINARY_MIME_TYPES
    )
