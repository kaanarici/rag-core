"""SearchOrchestrator end-to-end contract: vectors, sidecar, rerank, filters."""
...
from rag_core.search.types import RerankResult, SearchSidecarQuery, SparseVector, Term
...
def test_search_uses_precomputed_query_vectors_without_re_embedding() -> None:
    async def _run() -> None:
        ...
        orchestrator = _orchestrator(store=store, sparse=sparse, embedding=embedding)
...
def test_search_request_carries_query_plan_directly() -> None:
...
def test_empty_document_scope_returns_no_results_without_provider_search() -> None:
    async def _run() -> RecordingVectorStore:
        ...
        embedding = FakeEmbeddingProvider()
        sparse = FakeSparseEmbedder()
        orchestrator = _orchestrator(store=store, embedding=embedding, sparse=sparse)
...
def test_empty_allowlist_scope_returns_no_results_without_provider_search(
    ...
    for request in (
        SearchRequest(query="query", corpus_ids=[], namespace="space-1"),
        SearchRequest(
            query="query",
            corpus_ids=["corpus-1"],
            namespace="space-1",
...
def test_search_uses_first_sparse_channel_when_bm25_missing() -> None:
    async def _run() -> None:
        ...
        orchestrator = _orchestrator(store=store)
...
def test_dense_only_plan_does_not_require_sparse_query_vector() -> None:
    async def _run() -> None:
        store = RecordingVectorStore(search_results=[make_search_result()])
        sparse = FakeSparseEmbedder(empty_query_multi=True)
        orchestrator = _orchestrator(store=store, sparse=sparse)
...
def test_sparse_only_plan_discards_unused_precomputed_dense_vector() -> None:
    async def _run() -> None:
        ...
        orchestrator = _orchestrator(store=store, embedding=embedding)
...
def test_search_reranks_results_and_falls_back_on_error() -> None:
    async def _run() -> None:
        ...
        results = await orchestrator.search(
            SearchRequest(
                query="query", corpus_ids=["corpus-1"], namespace="space-1", rerank=True
            )
        )
...
def test_search_propagates_scope_and_metadata_filters_to_sidecar() -> None:
    """Scope (corpus/content_type/document_ids) and metadata_filter both reach the sidecar
    ...
    async def _run() -> None:
        ...
        assert call.corpus_ids == ["corpus-1"]
...
def test_portable_lexical_sidecar_promotes_exact_and_trigram_matches() -> None:
    async def _run() -> None:
        ...
        orchestrator = _orchestrator(sidecar=sidecar)
        ...
        trigram_results = await orchestrator.search(
            SearchRequest(
                query="fox quary", corpus_ids=["corpus-1"], namespace="space-1"
            )
        )
...
def test_trigram_sidecar_does_not_outrank_stronger_vector_hit() -> None:
    async def _run() -> None:
        ...
        results = await orchestrator.search(
            SearchRequest(query="alpha", corpus_ids=["corpus-1"], namespace="space-1")
        )
