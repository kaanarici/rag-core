...
def test_multi_query_fanout_caps_fused_candidates_before_rerank() -> None:
    async def _run() -> None:
        ...
        assert rerank.calls == [["original-hit", "variant-a-hit"]]
...
def test_query_expansion_rrf_runs_against_demo_qdrant() -> None:
    async def _run() -> None:
        async with build_demo_core(
            collection=f"query_expansion_{uuid.uuid4().hex}"
        ) as core:
