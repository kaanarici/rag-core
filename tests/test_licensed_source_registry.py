"""URL-list ingest binds URLs to a collection via LicensedSourceRegistry.

A URL whose host is not licensed for the target collection must be rejected
...
from rag_core.ingest.urls.sources import remote_url_source_items
...
def test_registry_accepts_exact_host_for_collection() -> None:
...
def test_registry_rejects_unlicensed_host_for_collection() -> None:
    request = RemoteUrlIngestRequest(
        namespace="ws",
        collection="licensed",
...
def test_registry_rejects_when_collection_has_no_entry() -> None:
    request = RemoteUrlIngestRequest(
        namespace="ws",
        collection="undeclared_corpus",
        urls=("https://docs.example.com/x.pdf",),
        licensed_source_registry=_registry(),
    )

    with pytest.raises(ValueError, match="has no entry for collection"):
        remote_url_source_items(request)


def test_registry_empty_collection_denies_everything() -> None:
    request = RemoteUrlIngestRequest(
        namespace="ws",
...
def test_registry_rejects_blank_collection_key() -> None:
