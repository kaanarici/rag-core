...
from rag_core.search.policy import CorpusPolicy, corpus_slug_for
...
from rag_core.search.providers.qdrant_lifecycle import (
...
# ---------------------------------------------------------------------------
# corpus_slug + resolve_collection_name + CorpusPolicy.collection_corpus_slug
# ---------------------------------------------------------------------------


def test_corpus_slug_lowercases_and_canonicalizes() -> None:
    assert corpus_slug_for("public") == "public"
    assert corpus_slug_for("Licensed") == "licensed"
    assert corpus_slug_for("Restricted!!!") == "restricted"
    assert corpus_slug_for("MIXED 2025/Q1") == "mixed_2025_q1"


def test_corpus_slug_rejects_empty_after_sanitization() -> None:
...
def test_resolve_collection_name_includes_corpus_slug() -> None:
    name = resolve_collection_name(
        base_name="rag_core_chunks",
        model_name="text-embedding-3-large",
        dimensions=3072,
        dimension_aware=True,
        corpus_slug="restricted",
    )
...
def test_corpus_policy_collection_slug_only_when_single_tier() -> None:
    bound = CorpusPolicy(
        bound_namespace="workspace_42",
        allowed_corpus_ids=frozenset({"restricted"}),
    )
    assert bound.collection_corpus_slug == "restricted"

    multi = CorpusPolicy(allowed_corpus_ids=frozenset({"public", "restricted"}))
    assert multi.collection_corpus_slug is None

    unbounded = CorpusPolicy()
    assert unbounded.collection_corpus_slug is None


def test_corpus_policy_collection_slug_sluggifies_uppercase_tier() -> None:
    policy = CorpusPolicy(allowed_corpus_ids=frozenset({"Licensed"}))
    assert policy.collection_corpus_slug == "licensed"
...
class _FakeConfigParams:
    def __init__(self, *, size: int, sparse_names: list[str] | None) -> None:
        from rag_core.search.providers.qdrant_shared import _DENSE_VECTOR_NAME
