...
def _processing_version(
    *,
    base_version: str = "rag_core_processing_v1",
...
def test_delete_document_requires_corpus_id() -> None:
    async def _run() -> None:
        ...
        try:
            with pytest.raises(
                ValueError, match="corpus_id is required for delete_document"
            ):
                await core.delete_document(
                    document_id="doc-1", namespace="team-space", corpus_id=""
                )
        finally:
            await core.close()
...
def test_explicit_document_ids_are_scoped_by_corpus_for_existing_checks() -> None:
    async def _run() -> None:
        # Shared document_id exists in a different corpus -> must not be treated as existing.
    ...
    async def _run() -> None:
        ...
        try:
            entry = await core.manifest_file(
                file_path, namespace="team-space", corpus_id="corpus-1"
            )
        finally:
            await core.close()
...
def test_close_attempts_all_resources_and_aggregates_failures() -> None:
    async def _run() -> None:
        ...
        assert "while closing RAGCore resource: vector_store" in notes
        assert "while closing RAGCore resource: embedding_cache" in notes
...
def test_close_re_raises_single_error_after_other_resources_are_closed() -> None:
    async def _run() -> None:
        ...
        assert embedding_cache.close_calls == 1
        assert chunk_context_cache.close_calls == 1
        assert "while closing RAGCore resource: vector_store" in getattr(
