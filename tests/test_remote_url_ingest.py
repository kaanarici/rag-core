...
from rag_core import remote_ingest_sources as remote_ingest_sources_module
...
from rag_core.remote_ingest_models import RemoteUrlIngestResult, RemoteUrlIngestSuccess
...
class _BootstrapErrorRemoteUrlCore(_FakeRemoteUrlCore):
    ...
    async def ingest_url(self, url: str, **kwargs: Any) -> IngestedDocument:
        ...
        return await super().ingest_url(url, **kwargs)
...
class _MixedStateRemoteUrlCore(_FakeRemoteUrlCore):
    async def ingest_url(self, url: str, **kwargs: Any) -> IngestedDocument:
        document = await super().ingest_url(url, **kwargs)
...
def test_build_remote_url_ingest_plan_requires_one_url_source(
    ...
    with pytest.raises(ValueError, match="no URLs found in inline URL list"):
        build_remote_url_ingest_plan(
            RemoteUrlIngestRequest(namespace="team-space", corpus_id="help")
        )
