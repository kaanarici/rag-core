    ...
    async def _run() -> None:
        ...
        await pipeline.run(
            _build_query(
                ...
                query_plan=default_query_plan(result_limit=20),
            ),
            ctx,
        )
...
def test_hybrid_retrieve_rejects_missing_sparse_embedder_before_dense_work() -> None:
    async def _run() -> tuple[FakeEmbeddingProvider, RecordingVectorStore]:
        embedding = FakeEmbeddingProvider()
        ...
        with pytest.raises(UnsupportedQueryStage, match="dense retrieval only"):
    ...
    embedding, store = asyncio.run(_run())
    assert embedding.embed_query_calls == []
