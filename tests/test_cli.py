...
``local_corpus``, and ``run_eval``. These tests focus on the seams the CLI
...
from rag_core.search.types import And, RerankBudget, Term
...
def _write_single_case(path: Path) -> None:
    path.write_text(
        json.dumps(
            {
                "query": "billing",
...
def test_eval_json_runs_against_configured_core(
    tmp_path: Path,
    fake_core: type[_FakeRAGCore],
    capsys: pytest.CaptureFixture[str],
) -> None:
    cases = tmp_path / "cases.jsonl"
    _write_single_case(cases)
    ...
    non_latency = {
        ...
        if k not in {"latency_ms", "latency_p95_ms", "throughput_qps"}
    }
    assert non_latency == {
        "mrr": 1.0,
        "ndcg_at_10": 1.0,
    ...
    assert payload["cases"][0]["retrieved_ids"] == ["doc-1"]
    # Validate the search call shape we depend on for downstream tests.
    instance = cast(_FakeRAGCore, fake_core._last_instance)  # type: ignore[attr-defined]
    call = instance.search_calls[0]
    assert call["query"] == "billing"
    assert call["namespace"] == "acme"
    assert call["corpus_ids"] == ["help"]
    assert call["limit"] == 10
    assert call["rerank"] is False
    assert call["rerank_budget"] is None
    assert call["query_plan"] is None


def test_eval_cases_directory_reports_cli_error_without_traceback(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    _expect_cli_error(
        ["eval", "--cases", str(tmp_path)],
        "cases path must be a JSONL file",
        capsys,
    )


def test_eval_quality_gate_passes_and_reports_json(
    tmp_path: Path,
    fake_core: type[_FakeRAGCore],
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    assert gate["thresholds"] == {
        "latency_ms": {"maximum": 100000.0},
        "latency_p95_ms": {"maximum": 100000.0},
        "mrr": {"minimum": 1.0},
        "recall_at_5": {"minimum": 1.0},
        "throughput_qps": {"minimum": 0.0},
    }


def test_eval_quality_gate_failure_returns_exit_1_with_json_payload(
    tmp_path: Path,
    fake_core: type[_FakeRAGCore],
    capsys: pytest.CaptureFixture[str],
) -> None:
    cases = tmp_path / "cases.jsonl"
    _write_single_case(cases)
    fake_core.search_handler = lambda self, **kw: [make_search_result(document_id="doc-2")]
...
def test_eval_quality_gate_text_output_reports_failure(
    tmp_path: Path,
    fake_core: type[_FakeRAGCore],
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    assert "- eval recall_at_5: 0.000 >= 1.000" in output


def test_eval_search_failure_reports_sanitized_json_failure(
    tmp_path: Path,
    fake_core: type[_FakeRAGCore],
    capsys: pytest.CaptureFixture[str],
) -> None:
...
def test_eval_search_failure_reports_sanitized_text_failure(
    tmp_path: Path,
    fake_core: type[_FakeRAGCore],
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    assert exit_code == 1
    output = capsys.readouterr().out
    assert "Search failures: 1" in output
    assert "raw provider" not in output


@pytest.mark.parametrize(
    ("argv_tail", "expected_error"),
    (
        (["--min-mrr", "1.1"], "--min-mrr must be a finite number between 0 and 1"),
        (["--min-mrr", "nan"], "--min-mrr must be a finite number between 0 and 1"),
        (["--max-mean-latency-ms", "-1"], "--max-mean-latency-ms must be finite and non-negative"),
        (["--max-mean-latency-ms", "inf"], "--max-mean-latency-ms must be finite and non-negative"),
        (["--max-p95-latency-ms", "-1"], "--max-p95-latency-ms must be finite and non-negative"),
        (["--max-p95-latency-ms", "inf"], "--max-p95-latency-ms must be finite and non-negative"),
        (["--min-throughput-qps", "-1"], "--min-throughput-qps must be finite and non-negative"),
        (["--min-throughput-qps", "nan"], "--min-throughput-qps must be finite and non-negative"),
    ),
)
def test_eval_quality_gate_rejects_invalid_thresholds_before_runtime_setup(
    argv_tail: list[str],
    expected_error: str,
    unexpected_core: type[_UnexpectedRAGCore],
    capsys: pytest.CaptureFixture[str],
) -> None:
    _expect_cli_error(["eval", "--cases", "cases.jsonl", *argv_tail], expected_error, capsys)
    assert unexpected_core.constructed is False


def test_eval_json_compares_rerank_runs(
    tmp_path: Path,
    fake_core: type[_FakeRAGCore],
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    def handler(self: _FakeRAGCore, **kw: Any) -> list[object]:
        if kw["rerank"]:
            return [
                make_search_result(document_id="doc-1"),
                make_search_result(document_id="doc-2"),
            ]
        return [
            make_search_result(document_id="doc-2"),
            make_search_result(document_id="doc-1"),
    ...
    assert exit_code == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["baseline"]["cases"][0]["retrieved_ids"] == ["doc-2", "doc-1"]
    assert payload["reranked"]["cases"][0]["retrieved_ids"] == ["doc-1", "doc-2"]
    assert payload["baseline"]["metrics"]["mrr"] == 0.5
    assert payload["reranked"]["metrics"]["mrr"] == 1.0
    assert payload["metric_deltas"]["mrr"] == 0.5
    assert payload["quality_gate"]["passed"] is True
    assert payload["quality_gate"]["failures"] == []

    baseline_call, rerank_call = _search_calls(fake_core)
    assert baseline_call["rerank"] is False
    assert baseline_call["rerank_budget"] is None
    assert baseline_call["query_plan"] is not None
    assert rerank_call["rerank"] is True
    budget = rerank_call["rerank_budget"]
    assert isinstance(budget, RerankBudget)
    assert (budget.candidate_count, budget.max_output, budget.timeout_seconds) == (2, 2, 1.5)
    assert budget.fallback_on_error is True
    assert rerank_call["query_plan"] == baseline_call["query_plan"]
...
@pytest.mark.parametrize(
    ...
    (
        (
            ["--rerank", "--compare-rerank"],
            "--rerank and --compare-rerank cannot be used together",
        ),
        (
            ["--rerank-candidates", "2"],
            "rerank budget flags require --rerank or --compare-rerank",
        ),
    ),
)
def test_eval_rejects_invalid_rerank_combinations(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
    argv_tail: list[str],
    expected_error: str,
    ...
    cases = tmp_path / "cases.jsonl"
    _write_single_case(cases)
    _expect_cli_error(["eval", "--cases", str(cases), *argv_tail], expected_error, capsys)


def test_eval_search_profile_passes_query_plan_to_core(
    tmp_path: Path,
    fake_core: type[_FakeRAGCore],
    capsys: pytest.CaptureFixture[str],
) -> None:
    cases = tmp_path / "cases.jsonl"
    _write_single_case(cases)
    expected_plan = search_profile("coverage", limit=10)
...
def test_eval_compare_search_profiles_reports_metrics_deltas_and_gate_failures(
    tmp_path: Path,
    fake_core: type[_FakeRAGCore],
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    def handler(self: _FakeRAGCore, **kw: Any) -> list[object]:
        if kw["query_plan"] == fast_plan:
            return [make_search_result(document_id="doc-1")]
        return [make_search_result(document_id="doc-2")]
    ...
    assert exit_code == 1
    payload = json.loads(capsys.readouterr().out)
    plans_seen = [call["query_plan"] for call in _search_calls(fake_core)]
    assert plans_seen == [balanced_plan, fast_plan]
    assert payload["baseline_profile"] == "balanced"
    assert payload["profiles"]["balanced"]["metrics"]["mrr"] == 0.0
    assert payload["profiles"]["fast"]["metrics"]["mrr"] == 1.0
    assert payload["quality_gate"]["passed"] is False
    assert payload["quality_gate"]["failures"][0]["scope"] == "balanced"


def test_eval_compare_search_profiles_rejects_compare_rerank_before_runtime_setup(
    unexpected_core: type[_UnexpectedRAGCore],
    capsys: pytest.CaptureFixture[str],
) -> None:
    _expect_cli_error(
        ...
        "--compare-search-profiles and --compare-rerank cannot be used together",
        capsys,
    )
    assert unexpected_core.constructed is False
...
@pytest.mark.parametrize(
    ...
    (
        (
            ["--compare-search-profiles", "balanced"],
            "--compare-search-profiles requires at least two profiles",
        ),
        (
            ["--compare-search-profiles", "balanced", "balanced"],
            "--compare-search-profiles entries must be unique: balanced",
        ),
    ),
)
def test_eval_compare_search_profiles_rejects_bad_profile_sets_before_runtime_setup(
    argv_tail: list[str],
    expected_error: str,
    unexpected_core: type[_UnexpectedRAGCore],
    capsys: pytest.CaptureFixture[str],
