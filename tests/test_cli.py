...
@pytest.mark.parametrize("argv", [["search"], ["search", "--context"]])
...
def test_doctor_json_reports_planned_core(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert pgvector["support_level"] == SUPPORT_FIRST_PARTY_OPTIONAL
...
def test_local_search_default_factory_uses_real_local_semantic_config(
    ...
    from rag_core import cli_local_search as cli_local_search_module
    ...
    monkeypatch.setattr(cli_local_search_module, "RAGCore", CapturingCore)
    ...
    config = cast(RAGCoreConfig, captured["config"])
    assert config == RAGCoreConfig.local()
...
def test_local_search_indexes_folder_and_returns_hits(
    ...
    exit_code = main(
        ["local-search", str(docs), "How do I pay invoices?", "--demo", "--json"]
    )
...
def test_local_search_skips_unsupported_inputs(
    ...
    exit_code = main(["local-search", str(docs), "retrieval", "--demo", "--json"])
...
def test_local_search_human_output_explains_skips_and_truncation(
    ...
    exit_code = main(
        ["local-search", str(docs), "retrieval", "--demo", "--max-files", "1"]
    )
...
def test_search_context_provider_bootstrap_error_uses_context_action(
    ...
    assert exc_info.value.code == 2
    error = capsys.readouterr().err
    assert "provider setup failed before search --context" in error
    assert "provider=openai" in error
    assert "api_key client option" not in error
    assert "OPENAI_API_KEY" in error
