...
def test_ci_workflow_runs_documented_validation_surface() -> None:
    ...
    assert workflow.count("if: matrix.python-version == '3.12'") == 4
    for command in ("uv sync --group dev", *CI_NON_PYTEST_CHECKS, *CI_PYTEST_TIERS):
        assert command in run_commands


def test_ci_builds_artifacts_without_a_distribution_workflow() -> None:
    root = Path(__file__).resolve().parents[1]
    workflow = (root / ".github" / "workflows" / "ci.yml").read_text(encoding="utf-8")
    docs = "\n".join(
        (root / path).read_text(encoding="utf-8")
        for path in ("docs/release.md", "scripts/README.md", "tests/README.md")
    )

    for expected in (
        "run: uv build",
        "run: uv run python scripts/check_dist_artifacts.py",
        "run: uv run python scripts/wheel_smoke.py",
    ):
        assert expected in workflow
    assert not (root / ".github" / "workflows" / "release-artifacts.yml").exists()
    assert "actions/upload-artifact" not in workflow
    assert "pypi" not in workflow.lower()
    assert "publish" not in workflow.lower()
    assert "source-only on GitHub" in docs
    assert "does not upload or publish" in docs
...
def test_operational_docs_do_not_reintroduce_orchestration_language() -> None:
    ...
    text = "\n".join(
        (root / path).read_text(encoding="utf-8")
        for path in (
            "docs-site/content/docs/self-host.mdx",
            "scripts/README.md",
            "pyproject.toml",
        )
    )
    ...
    assert "orchestration is the gateway's responsibility" in text
