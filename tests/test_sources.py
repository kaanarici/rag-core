...
from rag_core.ingest.local.planning import build_local_ingest_plan
from rag_core.local_search.models import LocalSearchRequest
from rag_core.local_search.planning import build_local_search_run_spec
...
def test_relative_local_sources_reject_symlinked_logical_cwd(
    ...
    with pytest.raises(ValueError, match="does not allow symlink paths"):
        build_local_ingest_plan(
            LocalIngestRequest(
                path=Path("target.md"), namespace="acme", collection="help"
            )
        )
