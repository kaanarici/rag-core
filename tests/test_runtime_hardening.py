...
def test_runtime_rejects_oversized_chunked_body_with_413(tmp_path: Path) -> None:
    ...
    def streamed_chunks() -> Any:
        yield b'{"query": "'
        yield b"y" * 256
        yield b'", "namespace": "acme", "collections": ["help"]}'
...
class _BlockingCore:
    """Engine stub that never finishes ingest_file until released."""
    ...
    with TestClient(app) as client:
        ...
        def fire_first() -> None:
            first_holder["response"] = client.post(
                "/v1/ingest",
                json={"path": str(doc1), "namespace": "acme", "collection": "help"},
            )
        ...
        try:
            ...
            second = client.post(
                "/v1/ingest",
                json={"path": str(doc2), "namespace": "acme", "collection": "help"},
            )
...
def test_cli_serve_accepts_non_loopback_with_flag(
    ...
    with caplog.at_level(_logging.WARNING, logger="rag_core.cli.commands.serve"):
...
def test_parse_ingest_request_enforces_bound_namespace() -> None:
    ...
    body = {"path": "/tmp/x.md", "namespace": "other", "collection": "help"}
    with pytest.raises(RuntimeRequestError) as excinfo:
...
def test_parse_retrieval_request_enforces_bound_namespace() -> None:
    ...
    body = {"query": "q", "namespace": "other", "collections": ["help"]}
    with pytest.raises(RuntimeRequestError) as excinfo:
...
def test_parse_delete_document_request_enforces_bound_namespace() -> None:
    ...
    body = {"namespace": "other", "collection": "help"}
    with pytest.raises(RuntimeRequestError) as excinfo:
        parse_delete_document_request(
            document_id="doc-1",
            payload=body,
...
def test_runtime_retrieval_rejects_empty_collections_via_contract() -> None:
    """The contract normalizer is what closes the tier-widening footgun at
    the HTTP boundary."""
    from rag_core.runtime.errors import RuntimeRequestError

    body = {"query": "q", "namespace": "acme", "collections": []}
    with pytest.raises(RuntimeRequestError) as excinfo:
        parse_retrieval_request(body)
    assert excinfo.value.details == {"field": "collections"}
