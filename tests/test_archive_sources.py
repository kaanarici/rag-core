...
def test_zip_archive_source_reader_plans_supported_members(tmp_path: Path) -> None:
    ...
    assert guide.member_bytes == b"# Guide\n\nHello"
...
def test_planned_archive_item_rejects_changed_member(tmp_path: Path) -> None:
    archive_path = tmp_path / "docs.zip"
    _write_zip(archive_path, {"docs/guide.md": b"first"})
    item = ZipArchiveSourceReader().read(archive_path).items[0]

    _write_zip(archive_path, {"docs/guide.md": b"other"})
...
@pytest.mark.parametrize(
    ("limits", "replacement", "message"),
    [
        (
            ArchiveLimits(max_entries=1, max_total_bytes=100),
            {"docs/guide.md": b"first", "docs/extra.md": b"x"},
            "max_entries",
        ),
        (
            ArchiveLimits(max_entries=2, max_total_bytes=5),
            {"docs/guide.md": b"first", "docs/extra.md": b"x"},
            "max_total_bytes",
        ),
    ],
)
def test_archive_session_revalidates_archive_limits_after_planning(
    tmp_path: Path,
    ...
    archive_path = tmp_path / "docs.zip"
    _write_zip(archive_path, {"docs/guide.md": b"first"})
    item = ZipArchiveSourceReader().read(
        archive_path,
        limits=limits,
    ).items[0]

    _write_zip(archive_path, replacement)

    with pytest.raises(ValueError, match=message):
        item.read_bytes()
    with pytest.raises(ValueError, match=message):
        with ZipArchiveReadSession(archive_path, limits=limits):
            pass
