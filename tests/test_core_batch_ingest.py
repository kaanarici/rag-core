...
def test_unchanged_ingest_uses_fast_skip_without_reparsing(tmp_path: Path) -> None:
    ...
    async def scenario() -> tuple[
        ...
        try:
            first = await core.add_file(doc, namespace="acme", collection="help")
            ...
            second = await core.add_file(doc, namespace="acme", collection="help")
...
def test_materialized_skip_mode_preserves_previous_prepare_behavior(tmp_path: Path) -> None:
    ...
    async def scenario() -> int:
        ...
        try:
            await core.add_file(doc, namespace="acme", collection="help")
            prepare_calls = 0
