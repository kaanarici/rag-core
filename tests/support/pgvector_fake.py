...
class PgVectorFakeConnection:
    ...
    async def execute(self, query: str, *args: object) -> str:
        ...
        if query.startswith("DELETE FROM") and "id = ANY" not in query:
            namespace, collection, document_id = _scope_args(args)
            self.rows = {
                point_id: row
                for point_id, row in self.rows.items()
                if not _matches_scope(row, namespace, collection, document_id)
            }
        return "OK"
    ...
    async def fetch(self, query: str, *args: object) -> list[Mapping[str, object]]:
        ...
        if "ORDER BY chunk_index ASC" in query:
            namespace, collection, document_id, indices = args
            wanted = set(_int_list(indices))
            rows = [
                _result_row(row, score=0.0)
                for row in self.rows.values()
                if _matches_scope(row, namespace, collection, document_id)
        ...
        if "ORDER BY dense <=>" in query:
            ...
            collections = args[2] if len(args) > 2 and isinstance(args[2], list) else None
            rows = [
                _result_row(row, score=0.8)
                for row in self.rows.values()
                if row.get("namespace") == namespace
                and (collections is None or row.get("collection") in collections)
            ]
    ...
    async def fetchrow(self, query: str, *args: object) -> Mapping[str, object] | None:
        ...
        for row in self.rows.values():
            if row.get("namespace") != namespace or row.get("collection") != collection:
                continue
...
def _scope_args(args: Sequence[object]) -> tuple[object, object | None, object | None]:
    ...
    return namespace, collection, document_id
...
def _matches_scope(
    ...
    if row.get("namespace") != namespace:
        return False
    if collection is not None and row.get("collection") != collection:
        return False
