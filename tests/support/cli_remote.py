...
from rag_core.fetch_security import safe_remote_document_key, safe_remote_source_url


class FakeRemoteEngine:
    _last_instance: ClassVar[FakeRemoteEngine | None] = None
    ...
    async def add_url(self, url: str, **kwargs: Any) -> IngestedDocument:
        ...
        return IngestedDocument(
            ...
            processing_version='{"base_version":"rag_core_processing_v4","source_type":"url"}',
...
def install_fake_remote_core(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    FakeRemoteEngine._last_instance = None
    monkeypatch.setattr(cli_module, "Engine", FakeRemoteEngine)


def require_fake_remote_core() -> FakeRemoteEngine:
    instance = FakeRemoteEngine._last_instance
    if instance is None:
        raise AssertionError("FakeRemoteEngine was not initialized")
    return instance
