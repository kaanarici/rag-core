...
class TurboPufferFakeNamespace:
    ...
    async def query(self, **kwargs: object) -> object:
        ...
        if kwargs.get("aggregate_by") is not None:
            matched = [
                row for row in self._rows.values() if _matches_filters(row, filters)
            ]
            return SimpleNamespace(
                rows=matched[:1],
        ...
        if isinstance(rank_by, tuple) and len(rank_by) == 3:
            ...
            if field == "vector" and mode == "ANN" and isinstance(vector, list):
                scored = [
                    (
                        row,
                        _cosine_distance(
                            vector, cast(list[float], row.get("vector") or [])
                        ),
                    )
                    for row in candidates
                ]
                scored.sort(key=lambda item: item[1])
                rows = [{**row, "$dist": dist} for row, dist in scored[:top_k]]
            ...
            if mode == "BM25":
                scored = [
                    ...
                    for row in candidates
                    if (
                        distance := _bm25_distance(
                            vector,
                            str(row.get(str(field)) or ""),
                        )
                    )
                    is not None
                ]
...
def _matches_filters(row: dict[str, object], filters: object) -> bool:
    ...
    if isinstance(filters, tuple) and len(filters) == 2:
        ...
        if op == "And":
            return all(
                _matches_filters(row, child) for child in cast(tuple[object], children)
            )
        if op == "Or":
            return any(
                _matches_filters(row, child) for child in cast(tuple[object], children)
...
def _bm25_distance(query: object, text: str) -> float | None:
