...
| Retrieval regression | `eval` | `tests/evals/pr_corpus/`: fixed vectors + Qdrant for pipeline/metric regression; `tests/evals/semantic_corpus/`: real local embeddings + Qdrant for a small end-to-end semantic-quality floor; `test_document_preparation_quality.py`: parser-contract metrics only, with no ranking claim |
...
| `tests/evals/semantic_corpus/` | Local semantic-quality regression corpus with near-miss distractors, duplicate and near-duplicate sources, scoped retrieval, and one generated XLSX source |
| `tests/evals/test_document_preparation_quality.py` | Cross-format parser-contract collection for retention, locators, chunk shape, OCR routing, figure precision, identity, latency, and archive access |
...
- `test_document_preparation_quality.py` is parser-contract evidence. It does not compute or claim semantic ranking metrics.
