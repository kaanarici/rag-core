...
) -> tuple[RAGCore, RecordingVectorStore, FakeFetchClient]:
...
def test_ingest_url_indexes_with_url_source_type_and_redacted_identity() -> None:
    async def _run() -> None:
        ...
        assert document.processing_version == (
            '{"base_version":"rag_core_processing_v1","source_type":"url"}'
        )
...
def test_ingest_url_emits_redacted_fetch_events() -> None:
    async def _run() -> None:
        ...
        assert started[0].corpus_id == "corpus-1"
        ...
        assert completed[0].corpus_id == "corpus-1"
...
def test_ingest_url_can_build_default_fetch_client_with_policy_and_limits(
    ...
    async def _run() -> None:
        ...
        monkeypatch.setattr(
            "rag_core.remote_sources.HttpFetchClient", RecordingHttpFetchClient
        )
        policy = ALLOW_HTTP_PRIVATE_POLICY
...
def test_ingest_url_skips_when_same_remote_content_is_already_indexed() -> None:
    async def _run() -> None:
        ...
        try:
            created = await core.ingest_url(
                "https://example.com/docs/guide?private=alpha",
                namespace="team-space",
                corpus_id="corpus-1",
                fetch_client=fetch_client,
            )
            repeated = await core.ingest_url(
                "https://example.com/docs/guide?private=alpha",
                namespace="team-space",
                corpus_id="corpus-1",
                fetch_client=fetch_client,
...
def test_ingest_url_distinguishes_redacted_query_identities() -> None:
    async def _run() -> None:
        ...
        try:
            first = await core.ingest_url(
                "https://example.com/export?id=1",
                namespace="team-space",
                corpus_id="corpus-1",
                fetch_client=fetch_client,
            )
            second = await core.ingest_url(
...
def test_ingest_url_reindexes_when_stored_processing_version_drifts() -> None:
    async def _run() -> None:
        ...
        try:
            ...
            reindexed = await core.ingest_url(
                "https://example.com/docs/guide?private=alpha",
                namespace="team-space",
                corpus_id="corpus-1",
                fetch_client=fetch_client,
