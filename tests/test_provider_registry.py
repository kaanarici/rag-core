...
from rag_core.documents.contextualizer_provider_names import CONTEXTUALIZER_PROVIDER_ORDER
from rag_core.documents.ocr_provider_names import OCR_PROVIDER_ORDER
import rag_core.search.providers.local_embedding as local_embedding_module
...
from rag_core.search.providers.cache_sqlite import CACHE_PROVIDER_ORDER
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (CONTEXTUALIZER_PROVIDERS, CONTEXTUALIZER_PROVIDER_ORDER),
...
def test_local_embedding_provider_registry_resolution_is_lazy(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fail_import() -> Any:
        raise AssertionError("runtime import should only happen on embed")

    monkeypatch.setattr(local_embedding_module, "_import_text_embedding", fail_import)

    assert "local" in EMBEDDING_PROVIDERS.names()
    provider = create_embedding_provider(provider="local")

    assert provider.model_name == LOCAL_EMBEDDING_MODEL
    assert provider.dimensions == LOCAL_EMBEDDING_DIMENSIONS
...
def test_local_embedding_provider_wraps_first_use_download_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        local_embedding_module,
        "_import_text_embedding",
        lambda: _FailingTextEmbedding,
    )
    provider = create_embedding_provider(provider="local")

    async def _run() -> None:
        with pytest.raises(RuntimeError) as exc_info:
            await provider.embed_texts(["first use initializes fastembed"])

        message = str(exc_info.value)
        assert LOCAL_EMBEDDING_MODEL in message
        assert "downloads this model once on first use" in message
        assert "provider='demo' for offline/CI smoke" in message

    asyncio.run(_run())


def test_local_embedding_provider_uses_fastembed_passage_and_query_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class _FakeTextEmbedding:
        ...
        def passage_embed(self, texts: list[str]) -> list[list[float]]:
            ...
            return [[1.0] * LOCAL_EMBEDDING_DIMENSIONS for _ in texts]

        def query_embed(self, queries: list[str]) -> list[list[float]]:
            self.query_calls.append(list(queries))
            return [[2.0] * LOCAL_EMBEDDING_DIMENSIONS for _ in queries]
    ...
    async def _run() -> None:
        text_vectors = await provider.embed_texts(["invoice payment", "vacation policy"])
        query_vector = await provider.embed_query("How do I pay a bill?")

        assert len(text_vectors) == 2
        assert text_vectors[0] == [1.0] * LOCAL_EMBEDDING_DIMENSIONS
        assert query_vector == [2.0] * LOCAL_EMBEDDING_DIMENSIONS
    ...
    assert instance.passage_calls == [["invoice payment", "vacation policy"]]
    assert instance.query_calls == [["How do I pay a bill?"]]
