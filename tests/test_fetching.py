...
def test_http_fetch_client_enforces_allowed_hosts_on_every_redirect_hop(
    http_origin: LocalHttpOrigin,
) -> None:
    http_origin.routes["/start"] = (
        302,
        {"Location": "https://other-host.example/final", "Content-Type": "text/plain"},
        b"",
    )
    ...
    client = HttpFetchClient(policy=policy)

    with pytest.raises(ValueError, match="not in allowed_hosts"):
        client.fetch(f"{http_origin.base_url}/start")


def test_http_fetch_client_refuses_initial_url_outside_allowed_hosts(
    http_origin: LocalHttpOrigin,
) -> None:
    policy = FetchSecurityPolicy(
        allowed_schemes=("https", "http"),
...
def test_http_fetch_client_allowed_hosts_empty_denies_everything(
    http_origin: LocalHttpOrigin,
) -> None:
    policy = FetchSecurityPolicy(
        allowed_schemes=("https", "http"),
...
def test_timed_out_resolutions_share_bounded_pool() -> None:
    def slow_resolver(_host: str, _port: int) -> tuple[str, ...]:
        time.sleep(0.2)
        return ("127.0.0.1",)
    ...
    for i in range(4):
        try:
            _local_fetch_client(
                limits=FetchLimits(timeout_seconds=0.02),
                resolver=slow_resolver,
            ).fetch(f"http://example-{i}.test/docs")
        except FetchError as exc:
            errors.append(exc)

    assert len(errors) == 4
    dns_threads = [t for t in threading.enumerate() if "rag-core-dns" in t.name]
    assert len(dns_threads) <= 8
