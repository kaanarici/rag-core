...
from rag_core.documents.pdf_page_locators import with_pdf_page_locators
...
def _reliable_span(chunk: PreparedChunk) -> tuple[int, int]:
...
def test_prepare_text_chunks_ignores_non_canonical_pdf_page_markup() -> None:
    """`# Page 1` (single-hash) is not the canonical OCR markup and must not bind."""
    text = (
        "# Page 1\n\nThis looks like a page heading but is a top-level title.\n\n"
    ...
    assert prepared
    assert all("page_number" not in chunk.metadata for chunk in prepared)


def test_prepare_text_chunks_persists_char_offsets_on_each_chunk() -> None:
    text = (
        "## Page 1\n\nFirst paragraph here for offset round-trip.\n\n"
        "## Page 2\n\nSecond paragraph here for offset round-trip.\n"
    )
    ...
    for chunk in prepared:
        ...
        assert end > start
        assert end <= len(text)
        # offset_reconstruction must NOT be present on reliably-resolved spans.
...
def test_prepare_pre_chunked_texts_yields_canonical_chunks() -> None:
    ...
    assert chunks[0].metadata["offset_reconstruction"] == "unreliable"
...
def test_pdf_page_locators_refuse_unreliable_chunk_offsets() -> None:
    [chunk] = with_pdf_page_locators(
        text="## Page 1\n\nAlpha\n\n## Page 2\n\nBeta",
        chunks=[
            PreparedChunk(
                chunk_index=0,
...
def test_prepare_text_chunks_overrides_embedding_text_when_supplied() -> None:
    ...
    assert chunk.sparse_text == "custom embedding text"
