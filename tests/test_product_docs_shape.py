...
def test_quickstart_install_story_matches_wheel_smoke_contract() -> None:
    ...
    assert "from rag_core.easy import index" in readme
    assert 'with index("./docs") as idx:' in readme
    assert "RAGCore(" in readme
    assert "await rag.ingest(" in readme
    assert "tools=[rag.tool()]" in readme
...
def test_agent_integration_doc_shape_is_linked_from_public_docs() -> None:
    ...
    assert "RAGCore.from_env" in agent_doc
    assert "tools=[rag.tool()]" in agent_doc
