...
def _entry(document_id: str) -> CorpusManifestEntry:
...
def test_append_manifest_jsonl_entry_if_stale_reuses_latest_entry_index(
    ...
    def count_parse(path_arg: Path, line_number: int, line: str) -> CorpusManifestEntry:
...
def test_append_manifest_jsonl_entry_failure_leaves_existing_file(
    ...
    monkeypatch.setattr("rag_core.manifest_jsonl._append_text_durable", fail_append)
...
def test_read_manifest_jsonl_entries_rejects_trailing_partial_append(
    ...
    monkeypatch.setattr("rag_core.manifest_jsonl._append_text_durable", partial_append)
...
def test_atomic_write_text_fsyncs_file_and_parent_directory(
    ...
    monkeypatch.setattr("rag_core.manifest_jsonl.os.fsync", record_fsync)
...
def test_manifest_jsonl_has_no_platform_specific_top_level_lock_imports() -> None:
    module = ast.parse(Path("src/rag_core/manifest_jsonl.py").read_text())
    top_level_imports = [
        alias.name
