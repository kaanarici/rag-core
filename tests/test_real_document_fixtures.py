...
def test_external_pdf_sections_survive_index_payload_and_context_pack() -> None:
    ...
    assert toolkit_snippet.locator.section_path == "Tika-Content Analysis Toolkit"
    assert toolkit_snippet.source.title == "testPDF.pdf"
    assert "Tika-Content Analysis Toolkit" in toolkit_snippet.header
    assert "Tika-Content Analysis Toolkit" in toolkit_snippet.prompt_header
    assert pack.source_previews[toolkit_snippet.rank - 1].locator_label == (
        "Tika-Content Analysis Toolkit, chunk 1"
    )
    assert pack.prompt_source_previews[toolkit_snippet.rank - 1].locator_label == (
        "Tika-Content Analysis Toolkit, chunk 1"
    )
