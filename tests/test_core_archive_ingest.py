...
def test_rag_core_ingest_archive_indexes_supported_zip_members(tmp_path: Path) -> None:
    ...
    async def scenario() -> tuple[
        ...
        try:
            ...
            rerun_result = await core.add_archive(
                archive_path,
                namespace="acme",
                collection="help",
                metadata={"team": "docs"},
    ...
    assert rerun_result.failed_count == 0
...
def test_rag_core_ingest_archive_failure_preserves_sanitized_message(
    ...
    class FailingCore:
        async def add_bytes(self, **_kwargs: object) -> Any:
...
def test_archive_ingest_streams_planned_members_on_demand(
    tmp_path: Path,
    ...
    assert result.succeeded_count == 2
    assert Counter(core.file_bytes) == Counter([b"# Guide", b"# Reference"])
    assert Counter(opened_members) == Counter(
        {
            "docs/guide.md": 2,
            "docs/reference.md": 2,
        }
...
def test_archive_ingest_indexes_central_directory_once_for_member_access(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    with zipfile.ZipFile(archive_path, "w") as archive:
        for index in range(member_count):
            archive.writestr(f"docs/{index:04d}.md", f"# Document {index}")

    infolist_calls = 0
    original_infolist = zipfile.ZipFile.infolist

    def counting_infolist(self: zipfile.ZipFile) -> list[zipfile.ZipInfo]:
    ...
    monkeypatch.setattr(zipfile.ZipFile, "infolist", counting_infolist)
    result = asyncio.run(
        ingest_zip_archive_with_core(
            core=CountingCore(),
            archive_path=archive_path,
    ...
    assert result.succeeded_count == member_count
    assert infolist_calls == 2


def test_archive_ingest_serializes_limit_sized_members_but_keeps_small_concurrency(
    tmp_path: Path,
) -> None:
    ...
    class TrackingCore:
        ...
        async def add_bytes(self, **kwargs: object) -> IngestedDocument:
            ...
            self.in_flight_bytes += len(file_bytes)
            self.peak_count = max(self.peak_count, self.in_flight_count)
            ...
            try:
                ...
                self.in_flight_bytes -= len(file_bytes)
            return IngestedDocument(
                document_id=cast(str, kwargs["filename"]),
                namespace=cast(str, kwargs["namespace"]),
                collection=cast(str, kwargs["collection"]),
                chunk_count=1,
                filename=cast(str, kwargs["filename"]),
    ...
    async def run_case(name: str, member_size: int) -> TrackingCore:
        ...
        result = await ingest_zip_archive_with_core(
            ...
            limits=ArchiveLimits(
                max_entries=4,
                max_entry_bytes=member_limit,
                max_total_bytes=4 * member_limit,
            ),
        )
        assert result.succeeded_count == 4
        return core

    async def scenario() -> tuple[TrackingCore, TrackingCore]:
        large, small = await asyncio.gather(
            run_case("large", member_limit),
            run_case("small", member_limit // 16),
        )
        return large, small

    large, small = asyncio.run(scenario())

    assert large.peak_count == 1
    assert large.peak_bytes == member_limit
    assert small.peak_count == 4
    assert small.peak_bytes == member_limit // 4
