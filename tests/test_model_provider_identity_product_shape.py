...
def test_model_provider_ids_have_single_adapter_or_config_owners() -> None:
    ...
    assert 'EMBEDDING_PROVIDERS.register("local"' not in consumers
    ...
    assert '("openai", "cohere", "voyage", "zeroentropy")' not in consumers
...
def test_embedding_provider_default_models_have_provider_module_owners() -> None:
    ...
    assert 'model: str = "embed-v4.0"' not in factory
