...
def _pool_state(pool: SqliteCacheConnectionPool) -> tuple[bool, int]:
    # Return (closed, connection_count) without leaking attribute-level
    # narrowing into the caller; mypy's reachability tracker otherwise
    # marks subsequent ``_closed``/``_connections`` reads unreachable.
    return pool._closed, len(pool._connections)
...
def test_provider_category_diagnostics_use_shared_cache_provider_names() -> None:
    diagnostics_source = Path(
        "src/rag_core/search/providers/provider_diagnostics.py"
    ).read_text(encoding="utf-8")

    assert "from .cache_sqlite import CACHE_PROVIDER_ORDER" in diagnostics_source
...
def test_create_sqlite_cache_factories_pass_eviction_bounds(tmp_path: Path) -> None:
...
def test_rag_core_resolves_embedding_cache_provider_from_config() -> None:
    """Config.ingest.embedding_cache_provider wires the cache via factory."""
    ...
    try:
        # Private surface. Engine has no public introspection for cache wiring yet.
        ...
        assert (
            ...
            == '{"base_version":"rag_core_processing_v3","source_type":"file"}'
        )
    finally:
...
def test_rag_core_close_closes_cache_resources(tmp_path: Path) -> None:
    ...
    # Per-task connection pool: pool is open before close().
    assert _pool_state(embedding_cache._pool) == (False, 0)
    assert _pool_state(chunk_context_cache._pool) == (False, 0)
    ...
    assert _pool_state(embedding_cache._pool) == (True, 0)
    assert _pool_state(chunk_context_cache._pool) == (True, 0)
