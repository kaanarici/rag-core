...
from rag_core.search.providers.qdrant_query import (
...
def test_adapter_search_uses_dense_default_query_plan() -> None:
    client = _FakeQdrantClient(existing_names=["docs"], sparse_names=["bm25"])
    ...
    call = client.query_points_calls[0]
    assert isinstance(call["query"], rest.NearestQuery)
    assert call["using"] == ""
    assert call["prefetch"] is None
    assert call["limit"] == 5


def test_direct_qdrant_default_plan_is_dense_with_primary_sparse_available() -> None:
    query = _make_query(query_plan=None, limit=5)
...
def test_direct_qdrant_default_plan_is_dense_with_custom_sparse_available() -> None:
    query = _make_query(
        query_plan=None,
        limit=5,
...
def test_adapter_default_query_plan_is_dense_with_primary_sparse_channel() -> None:
...
def test_adapter_default_query_plan_is_dense_with_other_sparse_channels() -> None:
    store = _store_with_fake_client(
