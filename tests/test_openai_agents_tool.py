...
def test_build_retrieve_context_tool_rejects_core_without_context(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _install_fake_agents(monkeypatch)

    with pytest.raises(TypeError, match="context"):
        build_retrieve_context_tool(object(), namespace="acme", collections=["help"])  # type: ignore[arg-type]
...
def test_retrieve_context_tool_can_hide_strategy_options(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _install_fake_agents(monkeypatch)
    ...
    assert "rerank" not in inspect.signature(tool_fn).parameters
    assert "use_lexical_search" not in inspect.signature(tool_fn).parameters
    asyncio.run(tool_fn(query="billing"))
    assert core.calls[0]["rerank"] is False
    assert core.calls[0]["use_lexical_search"] is False
...
def test_build_retrieve_context_tool_validates_config(
    ...
    base: dict[str, Any] = {"namespace": "acme", "collections": ["help"]}
    base.update(kwargs)
...
def test_retrieve_context_tool_rejects_values_outside_contract_bounds(
    ...
    _install_fake_agents(monkeypatch)
    core = _FakeCore(_FakePack(text="", payload={}))
    tool_fn = build_retrieve_context_tool(core, namespace="acme", collections=["help"])
