...
from rag_core.ingest.sources.local import expand_supported_local_files
...
def test_expand_supported_local_files_excludes_ocr_required_image(
    tmp_path: Path,
) -> None:
...
def test_search_path_subparser_accepts_trace_jsonl() -> None:
...
def test_add_help_describes_supported_file_filter(
    ...
    assert "Add one unambiguous source" in normalized
    assert (
        "Supported file, directory, shell glob, .zip archive, or HTTP(S) URL"
        in normalized
    )
    assert "--url-list" in help_text
    assert "--dry-run" in help_text


def test_query_subparser_accepts_repeating_collection() -> None:
    parser = _build_parser()
    args = parser.parse_args(
        [
            "search",
    ...
    assert args.collection == ["help", "internal"]
...
def test_core_ingestor_writes_manifest_entry(tmp_path: Path) -> None:
    ...
    asyncio.run(go())
    entries = read_entries(manifest_dir, namespace="acme", collection="help")
