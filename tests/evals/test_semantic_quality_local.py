...
def test_semantic_corpus_fixture_shape() -> None:
    ...
    corpus_ids = {doc.document_id for doc in corpus}
    ...
    assert all(case.corpus_ids == (_CORPUS_ID,) for case in cases)
    assert all(set(case.expected_ids) <= corpus_ids for case in cases)
...
async def _run_semantic_eval(
    ...
    async with RAGCore(config) as core:
        ...
        if degradation is not None:
            eval_core = cast(RAGCore, _DegradedSearchCore(core, degradation))
...
class _DegradedSearchCore:
    def __init__(self, core: RAGCore, degradation: DegradationMode) -> None:
        self._core = core
...
def _semantic_config() -> RAGCoreConfig:
    config = RAGCoreConfig.local()
    return replace(
        config,
        qdrant=replace(
            config.qdrant,
            collection=f"rag_core_semantic_eval_{uuid.uuid4().hex}",
        ),
    )


def _assert_local_embedding_runtime(core: RAGCore) -> None:
