...
def test_qdrant_store_declares_spec_query_plan_support() -> None:
    ...
    try:
        assert (
            store.capabilities.query_plan
            == QDRANT_VECTOR_STORE_CAPABILITY_SPEC.query_plan
        )
...
def test_turbopuffer_store_declares_bm25_hybrid_query_plan_support() -> None:
...
def test_pgvector_store_declares_dense_only_query_plan_support() -> None:
    ...
    assert store.capabilities.query_plan == PGVECTOR_VECTOR_STORE_CAPABILITY_SPEC.query_plan
    assert store.capabilities.query_plan.dense is True
