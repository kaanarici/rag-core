...
from rag_core.search.providers.turbopuffer_query_plan import (
    TURBOPUFFER_SEARCH_MODE_DENSE,
    TURBOPUFFER_SEARCH_MODE_HYBRID_RRF,
...
def test_primary_sparse_channel_has_single_owner() -> None:
    ...
    assert (
        'or "bm25"'
        not in sources["src/rag_core/search/providers/turbopuffer_search.py"]
    )
...
def test_turbopuffer_search_execution_modes_have_single_owner() -> None:
    ...
    assert TURBOPUFFER_SEARCH_MODE_DENSE == "dense"
    assert TURBOPUFFER_SEARCH_MODE_HYBRID_RRF == "hybrid_rrf"
    assert TURBOPUFFER_SEARCH_MODE_SPARSE_KNN == "sparse_knn"
    ...
    assert "mode: TurboPufferSearchMode" in owner

    consumer = sources["src/rag_core/search/providers/turbopuffer_search.py"]
    assert "TURBOPUFFER_SEARCH_MODE_DENSE" in consumer
    assert "TURBOPUFFER_SEARCH_MODE_SPARSE_KNN" in consumer
