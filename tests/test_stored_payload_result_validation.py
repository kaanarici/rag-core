...
def test_build_stored_payload_round_trips_chunk_char_offsets_to_result() -> None:
    payload = build_stored_payload(
        namespace="acme",
        collection="help",
        document_id="doc-1",
    ...
    # Span persisted directly even with no section_info.
    assert payload["start_char"] == 42
    assert payload["end_char"] == 53

    result = payload_to_result(point_id="point-1", payload=payload, score=0.5)
    assert result.start_char == 42
    assert result.end_char == 53
    # Spans live on the SearchResult, not in opaque metadata.
    assert "start_char" not in result.metadata
    assert "end_char" not in result.metadata


def test_build_stored_payload_propagates_section_info_span_when_chunk_span_absent() -> None:
    payload = build_stored_payload(
        namespace="acme",
        collection="help",
        document_id="doc-1",
        ...
        document_path=None,
        chunk_index=0,
        chunk_text="row block",
        chunk_token_count=2,
        payload_text="row block",
        content_type=ContentType.DOCUMENT,
        embedding_model="text-embedding-3-small",
        chunker_strategy="prechunked",
        title=None,
        section_info={"start_char": 7, "end_char": 16, "sheet_name": "Signals"},
    )

    assert payload["start_char"] == 7
    assert payload["end_char"] == 16
    assert payload["sheet_name"] == "Signals"
