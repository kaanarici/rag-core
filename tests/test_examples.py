...
def test_ask_folder_example_is_copyable_local_path() -> None:
    ...
    assert "RAGCoreConfig" not in source
    assert ".ingest_files(" not in source
...
def test_vercel_ai_sdk_example_matches_public_tool_payload() -> None:
    ...
    docs = Path("docs-site/content/docs/embed.mdx").read_text(encoding="utf-8")
    assert "stable AI SDK v6 tool contracts verified by" in docs
    assert "`./scripts/verify_vercel_ai_sdk_example.sh`" in docs
    assert "current published" in docs
    assert "`ai@^6.0.0` TypeScript declarations" in docs
    assert "TypeScript declarations" in docs
    assert "`text` tool output" in docs
    assert "The streamed" in docs
    assert "branch follows the installed `fullStream` declaration" in docs
    assert "`tool-error` and `error` stream parts" in docs
    assert "Verify the current Vercel AI" in docs
    assert "Gateway model" in docs
    assert "list before copying it into an application" in docs
    assert "declaration compatibility only" in docs
    assert "not a v7 beta contract" in docs

    expectations = Path("docs-site/content/docs/expectations.mdx").read_text(
        encoding="utf-8"
    )
...
def test_examples_use_prompt_safe_text() -> None:
    for path in (
        Path("docs-site/content/docs/embed.mdx"),
