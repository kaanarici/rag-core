...
def test_ask_folder_example_is_copyable_local_path() -> None:
    source = Path("examples/ask_folder.py").read_text(encoding="utf-8")
    nonblank = [line for line in source.splitlines() if line.strip()]

    assert len(nonblank) <= 12
    assert "from rag_core import index" in source
    assert "with index(" in source
    assert ".context(" in source
    assert "Config" not in source
    assert ".add(" not in source
    assert "rag_core.demo" not in source
    assert "ChunkingConfig" not in source


def test_easy_index_empty_folder_raises_clean_error(tmp_path: Path) -> None:
    from rag_core import index

    with pytest.raises(RuntimeError) as excinfo:
        index(tmp_path)
    message = str(excinfo.value)
    assert "indexed no files" in message
    # The day-one facade must not leak lower-level glob/ingest guidance.
    assert "glob" not in message
    assert "supported files matched" not in message


def test_easy_index_rejects_an_invalid_scope_before_initializing_providers() -> None:
    from rag_core import index

    with pytest.raises(ValueError, match="collection must be a non-empty string"):
        index("examples/demo_corpus", collection="   ")


@pytest.mark.live
def test_ask_folder_example_runs_with_easy_index_path() -> None:
    ...
    assert "Citations:" in result.stdout
    assert "invoice" in result.stdout.lower() or "pay" in result.stdout.lower()
...
def test_vercel_ai_sdk_example_matches_public_tool_payload() -> None:
    ...
    # Public docs describe the prompt-safe projection the tool payload depends on:
    # `to_prompt_payload()` carries rank-local previews and drops the source hash
    # and other private identifiers.
    context_docs = " ".join(
        Path("docs-site/content/docs/context.mdx")
        .read_text(encoding="utf-8")
        .split()
    )
    assert "to_prompt_payload" in context_docs
    assert "the locator omits `source_hash`" in context_docs
...
def test_configured_retrieval_ci_wording_is_precise() -> None:
    ...
    assert "not executed by default CI because it requires credentials" in example
    assert "source-checked and packaged" in example
    assert "Not run in CI" not in example
    assert "Not part of default CI" not in example
...
def test_python_examples_teach_public_root_config_import() -> None:
    for path in Path("examples").glob("*.py"):
        ...
        assert "from rag_core.core_models import Config" not in source
        assert "from rag_core.core import Engine" not in source
        assert "from rag_core.ingest.urls.document_keys import" not in source
...
def test_corpus_lifecycle_example_tracks_manifest_and_delete() -> None:
    async def go() -> None:
        ...
        try:
            ...
            assert store.search_calls[0].collections == ["help-center"]
...
def test_retrieval_eval_module_prints_redacted_report() -> None:
    ...
    assert payload["run"]["purpose"] == "wiring_only_demo_embeddings"
...
def test_configured_eval_help_smoke_without_model_download() -> None:
    result = subprocess.run(
        [sys.executable, "-m", "examples.configured_eval", "--help"],
        cwd=Path.cwd(),
        env=_module_env(),
        capture_output=True,
        text=True,
    ...
    assert result.returncode == 0
    assert "corpus_dir" in result.stdout
    assert "--provider {local,openai}" in result.stdout


@pytest.mark.live
def test_configured_eval_runs_with_local_provider() -> None:
    if os.environ.get("RAG_CORE_RUN_FASTEMBED_LIVE") != "1":
        pytest.skip(
            "set RAG_CORE_RUN_FASTEMBED_LIVE=1 to run the FastEmbed live smoke "
            "(downloads the local embedding model on first use)"
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "examples.configured_eval",
            "examples/demo_corpus",
            "examples/eval_cases.jsonl",
        ],
        cwd=Path.cwd(),
        env=_module_env(),
        capture_output=True,
    ...
    assert result.returncode == 0, result.stderr
    assert payload["run"]["mode"] == "local"
    assert payload["run"]["embedding_model"] == "BAAI/bge-small-en-v1.5"
