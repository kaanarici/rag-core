...
def test_extraction_requests_cli_page_markers(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    commands: list[list[str]] = []

    def run(command: list[str], file_bytes: bytes) -> dict[str, object]:
        commands.append(command)
        assert file_bytes == b"%PDF-1.7"
        return _extraction_payload(markdown="<!-- Page 1 -->\n\n# Report")

    monkeypatch.setattr(pdf_inspector_runtime, "run_pdf_inspector", run)

    result = pdf_inspector_module.extract_pdf_with_inspector(b"%PDF-1.7")

    assert result is not None
    assert commands == [["pdf2md", "--pages", "--json"]]
...
def test_extraction_preserves_ocr_field_presence(
    monkeypatch: pytest.MonkeyPatch,
    explicit: bool,
) -> None:
    ...
    assert result is not None
    assert result.pages_needing_ocr == []
    assert result.has_explicit_ocr_page_info is explicit
...
def test_cli_payload_page_count_bound(
    monkeypatch: pytest.MonkeyPatch,
    page_count: int,
    accepted: bool,
) -> None:
...
def test_extraction_rejects_invalid_pages_needing_ocr(
    ...
    monkeypatch.setattr(
        ...
        lambda command, file_bytes: _extraction_payload(
            pages_needing_ocr=invalid_value
        ),
    )
...
def test_wheel_process_result_normalizes_to_inspector_contract(
    ...
    assert result is not None
    assert result.detection.has_explicit_ocr_page_info is True
...
@pytest.mark.parametrize("pdf_type", ["scanned", "ocr-only"])
def test_wheel_process_result_preserves_missing_ocr_page_field(
    pdf_type: str,
) -> None:
    ...
    assert result.detection.has_explicit_ocr_page_info is False
    assert result.detection.pages_needing_ocr == []


def test_wheel_process_result_rejects_nonempty_invalid_only_ocr_pages(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        pdf_inspector_module,
        "_import_pdf_inspector_wheel",
        lambda: SimpleNamespace(
            process_pdf_bytes=lambda file_bytes: {
                "pdf_type": "mixed",
                "page_count": 2,
                "pages_needing_ocr": [-1, 99],
                "confidence": 0.9,
                "markdown": "partial wheel markdown",
            }
        ),
    )

    assert pdf_inspector_module.process_pdf_with_inspector_wheel(b"%PDF-1.7") is None


def test_wheel_ocr_aliases_union_after_earlier_empty_alias() -> None:
    result = pdf_inspector_module._process_wheel_result(
        {
            "pdf_type": "mixed",
            "page_count": 3,
    ...
    assert result.detection.pages_needing_ocr == [0, 2]
    assert result.detection.has_explicit_ocr_page_info is True


@pytest.mark.parametrize("later_alias", [[-1, 99], "not-a-list"])
def test_wheel_earlier_empty_alias_does_not_mask_invalid_later_alias(
    later_alias: object,
) -> None:
    with pytest.raises(ValueError):
        pdf_inspector_module._process_wheel_result(
            {
                "pdf_type": "mixed",
                "page_count": 2,
                "pages_needing_ocr": [],
                "ocr_page_indices": later_alias,
                "confidence": 0.9,
                "markdown": "partial wheel markdown",
            }
        )
...
def test_wheel_partially_valid_alias_retains_valid_indices() -> None:
    result = pdf_inspector_module._process_wheel_result(
        {
            "pdf_type": "mixed",
            "page_count": 2,
            "pages_needing_ocr": [],
            "ocr_page_indices": [-1, 0, 99],
            "confidence": 0.9,
            "markdown": "partial wheel markdown",
        }
    )

    assert result.detection.pages_needing_ocr == [0]


def test_wheel_page_count_bound() -> None:
    accepted = pdf_inspector_module._process_wheel_result(
        {
            "pdf_type": "text",
            "page_count": MAX_PDF_PAGE_COUNT,
            "confidence": 0.9,
            "markdown": "wheel markdown",
        }
    )

    assert accepted.detection.page_count == MAX_PDF_PAGE_COUNT
    with pytest.raises(ValueError):
        pdf_inspector_module._process_wheel_result(
            {
                "pdf_type": "text",
                "page_count": MAX_PDF_PAGE_COUNT + 1,
                "confidence": 0.9,
                "markdown": "wheel markdown",
            }
        )
