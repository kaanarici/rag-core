...
def test_cli_top_level_help_excludes_removed_commands(
    ...
    assert "rag-core local-search <folder>" in output
...
def test_configured_cli_docs_keep_query_commands_on_same_store_and_known_model() -> None:
    ...
    assert (
        'search --context "billing policy" --namespace acme --corpus-id help \\\n'
        "  --qdrant-url http://127.0.0.1:6333 \\\n"
        "  --embedding-provider openai --embedding-model text-embedding-3-small"
        in docs
    )
    assert "Known local and OpenAI embedding models infer dimensions automatically" in docs
    assert (
