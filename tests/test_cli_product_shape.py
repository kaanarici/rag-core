...
def test_retrieval_default_docs_distinguish_profiles_from_lexical_sidecar() -> None:
    ...
    # The common path defaults to the dense baseline; hybrid remains an
    # explicit profile for sparse-capable indexes.
    assert "default mode is dense retrieval" in docs
    assert "balanced" in docs
    assert "hybrid RRF profile" in docs
