...
def test_retrieval_default_docs_distinguish_profiles_from_lexical_sidecar() -> None:
    ...
    # Default retrieval is dense; hybrid remains an explicit advanced profile.
    assert "Default retrieval is dense-only" in docs
    assert "dense-only `fast`" in docs
    ...
    # Guardrail: do not overclaim that hybrid is universally on.
    assert "explicit advanced" in docs
    assert (
