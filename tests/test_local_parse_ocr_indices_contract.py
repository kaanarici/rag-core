...
def test_local_parse_empty_ocr_output_requires_real_page_indices(
    ...
    monkeypatch.setattr(
        converters_module, "convert_file", convert_with_boolean_only_index
    )

    with pytest.raises(
        local_parse.LocalParseError, match="Converter returned empty output"
    ):
        asyncio.run(
            local_parse.parse_file_bytes(
                file_bytes=b"%PDF-1.7",
...
def test_gemini_pdf_subset_uploads_only_requested_pages_and_reports_original_indices(
    ...
    def fake_load_json(req: Request) -> dict[str, object]:
        ...
        try:
            ...
            doc.close()
        assert "original document page numbers in order: 2, 4." in _gemini_prompt(body)
        return {
            "candidates": [
                {
                    "content": {
...
def test_gemini_pdf_empty_and_full_range_requests_upload_original_bytes(
    ...
    def fake_load_json(req: Request) -> dict[str, object]:
        ...
        assert _gemini_prompt(body) == gemini_command._build_prompt([0, 1])
        return {
            "candidates": [
                {
                    "content": {
    ...
    assert output["metadata"]["ocr_page_indices_ignored"] is bool(
