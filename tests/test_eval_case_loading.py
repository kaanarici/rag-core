...
def test_load_cases_rejects_removed_expected_chunk_ids(tmp_path: Path) -> None:
    path = tmp_path / "cases.jsonl"
    path.write_text(
        json.dumps(
            {
    ...
    with pytest.raises(ValueError, match="expected_ids must be a non-empty string array"):
        load_cases(path)


def test_load_cases_accepts_scoped_no_answer_controls(tmp_path: Path) -> None:
    path = tmp_path / "cases.jsonl"
    path.write_text(
        json.dumps(
            {
                ...
                "namespace": "acme",
                "collections": ["help"],
                "document_ids": ["internal-doc"],
                "expected_ids": [],
                "forbidden_ids": ["private-source"],
    ...
    assert case.document_ids == ("internal-doc",)
    assert case.forbidden_ids == ("private-source",)
    assert case.expected_ids == ()
    assert case.expect_no_results is True
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            ...
            "collections must be a non-empty string array",
        ),
        (
            {
                "query": "q",
...
def test_load_cases_rejects_duplicate_json_object_keys(tmp_path: Path) -> None:
    path = tmp_path / "cases.jsonl"
    path.write_text(
        '{"query":"q1","query":"q2","namespace":"acme","collections":["help"],"expected_ids":["doc"]}',
        encoding="utf-8",
    )
