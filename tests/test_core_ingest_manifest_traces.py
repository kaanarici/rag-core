...
def test_unchanged_ingest_without_manifest_preserves_indexed_source_identity(
    ...
    [entry] = read_entries(manifest_directory, namespace="acme", collection="help")
...
def test_failed_reindex_restores_previous_manifest_shape(tmp_path: Path) -> None:
    ...
    manifest_directory = tmp_path / "manifest"
    previous_entry = CollectionManifestEntry(
        document_id="doc-1",
        namespace="acme",
        collection="help",
        document_key="doc.md",
...
def test_existing_document_manifest_write_failure_surfaces_and_leaves_pending_replay(
    ...
    # The failing manifest write surfaces directly: no silent retry of the
    # identical call (which would only fail again on a real disk error). The
    # staged write (1) plus the failed final write (2) are the only attempts.
    ...
    async def run() -> tuple[EmptyPayloadIndexer, FailingSidecar]:
        ...
        async def prepare_bytes(
            *,
            ...
            del file_bytes, namespace, collection, document_id
...
def test_delete_document_keeps_manifest_when_index_delete_fails(
    ...
    """Right-to-forget order is vector-store-FIRST.

    If the vector store delete fails, the sidecar is not touched (avoids the
    leak where the sidecar's lexical projection is purged while the canonical
    retrieval surface still serves the doc). The manifest entry survives so
    a recovery replay sees the document and can finish the purge.
    """
    manifest_directory = tmp_path / "manifest"
    ...
    assert indexer.delete_document_calls == 1
    # Vector store failed first; sidecar must NOT have been touched.
...
def test_delete_document_keeps_manifest_when_manifest_delete_fails(
    ...
    """Manifest is purged LAST so an earlier crash leaves the entry readable.
...
def test_delete_document_keeps_index_and_manifest_when_sidecar_delete_fails(
    ...
    assert sidecar.deleted == [("acme", "doc-1")]
    # Vector store ran first; failure happened on the SIDECAR stage.
    assert indexer.delete_document_calls == 1
    entries = read_entries(manifest_directory, namespace="acme", collection="help")
    assert [entry.document_id for entry in entries] == ["doc-1"]
    journal_file = manifest_directory / ".delete_recovery.jsonl"
