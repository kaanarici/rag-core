...
class _StartedSpan:
    ...
    def __enter__(self) -> "_StartedSpan":
        return self
...
class _FakeTracer:
    def __init__(self) -> None:
        self.started_spans: list[_StartedSpan] = []
    ...
    def start_as_current_span(
        ...
        *,
        ...
        span = _StartedSpan(name, attributes)
...
def test_open_telemetry_sink_sanitizes_provider_model_and_stage_labels(
    ...
    sink.emit(
        EmbedCompleted(
            ...
            model="ghp_" + "abcdefghijklmnopqrstuvwxyz123456",
        )
    )
...
def test_open_telemetry_sink_starts_span_without_recording_current_span(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    current_span = _FakeNonRecordingSpan()
    tracer = _install_fake_opentelemetry(monkeypatch, current_span)
    ...
    assert current_span.events == []
    [span] = tracer.started_spans
    assert span.name == "search.planned"
    assert span.ended is True
    assert span.attributes["rag_core.limit"] == 3
    assert span.attributes["rag_core.channels"] == [_DENSE_PRIMARY_CHANNEL]


def test_open_telemetry_sink_counts_fallback_span_errors(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class BrokenStartedSpan:
        def __enter__(self) -> "BrokenStartedSpan":
            return self
    ...
    class BrokenTracer:
        def start_as_current_span(
            ...
            *,
            ...
            del name, attributes
            return BrokenStartedSpan()

    _install_fake_opentelemetry(
        monkeypatch,
        _FakeNonRecordingSpan(),
        tracer=BrokenTracer(),
    )
    sink = OpenTelemetrySink()
...
def test_open_telemetry_sink_reports_missing_optional_dependency(
    ...
    with pytest.raises(ImportError, match="opentelemetry optional extra"):
        OpenTelemetrySink()
...
def test_open_telemetry_sink_real_sdk_spanless_fallback_creates_span(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    trace_api: Any = pytest.importorskip("opentelemetry.trace")
    sdk_trace: Any = pytest.importorskip("opentelemetry.sdk.trace")
    sdk_export: Any = pytest.importorskip("opentelemetry.sdk.trace.export")
    exported_spans: list[Any] = []
    ...
    monkeypatch.setattr(trace_api, "get_current_span", lambda: trace_api.INVALID_SPAN)
    try:
        OpenTelemetrySink().emit(
            SearchPlanned(
                namespace="acme",
    ...
    [span] = exported_spans
    assert span.name == "search.planned"
    assert dict(span.attributes)["rag_core.limit"] == 3
